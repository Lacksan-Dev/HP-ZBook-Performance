[CmdletBinding()]
param([string]$ScriptPath=(Join-Path $PSScriptRoot 'Invoke-LogiTuneRunCalibration.ps1'))
...
$required=@('SupportsShouldProcess=$true',"'Check','Capture','DryRun','Apply','Verify','VerifyReboot','Rollback'",'DoNotExpandEnvironmentNames','Test-LogiTuneIdentity','ConvertTo-Json -Compress','Rollback refused because the registration already exists')
...
[pscustomobject]@{Experiment='EXP-034';ParsePassed=$true;ContractTokens=$required.Count;ForbiddenOperationsFound=0;LiveChangesApplied=$false}
