...
function Save-State($svc){
  ...
  if($parent -and -not(Test-Path -LiteralPath $parent)){New-Item -ItemType Directory -Path $parent -Force|Out-Null}
  $state=[ordered]@{schema=1;experiment='EXP-031';computer=$env:COMPUTERNAME;service=$svc.Name;displayName=$svc.DisplayName;path=$svc.PathName;startMode=$svc.StartMode;state=$svc.State;capturedUtc=(Get-Date).ToUniversalTime().ToString('o')}
  $state|ConvertTo-Json -Depth 6|Set-Content -LiteralPath $StatePath -Encoding UTF8
  Write-Event 'state-captured' @{service=$svc.Name;displayName=$svc.DisplayName;startMode=$svc.StartMode;state=$svc.State;path=$svc.PathName}
  $state
}
...
function Read-State {
  ...
  if([string]::IsNullOrWhiteSpace([string]$s.path) -or [string]::IsNullOrWhiteSpace([string]$s.displayName) -or [string]::IsNullOrWhiteSpace([string]$s.startMode)){throw 'State content refused'}
  $s
}
...
try {
  $svc=Get-Candidate
  switch($Action){
    'Check' {Write-Event 'supported' @{service=$svc.Name;displayName=$svc.DisplayName;path=$svc.PathName;startMode=$svc.StartMode;state=$svc.State};$svc|Select-Object Name,DisplayName,PathName,StartMode,State}
