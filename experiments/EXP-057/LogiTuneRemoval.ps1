...
function Get-LogiTuneProduct {
    $matches = foreach ($root in $uninstallRoots) {
        Get-ItemProperty -Path $root -ErrorAction SilentlyContinue | Where-Object {
            ...
            $_.Publisher -match '^Logitech' -and
...
function Get-Support {
    ...
    [pscustomobject]@{
        Supported = ($os.Caption -match 'Windows 11' -and $products.Count -eq 1 -and $installerExists)
...
function Assert-Supported {
    ...
    if ($Support.ProductCount -ne 1) { throw 'Exactly one Logitech-published Logi Tune uninstall product is required.' }
...
function Test-Removed { return (Get-LogiTuneProduct).Count -eq 0 }
    ...
    switch ($Action) {
        ...
        'Rollback' {
            ...
            if ($PSCmdlet.ShouldProcess($state.product.displayName,'Reinstall exact captured product')) {
                ...
                $restored = Get-LogiTuneProduct
                if ($restored.Count -ne 1 -or $restored[0].DisplayVersion -ne $state.product.displayVersion) { throw 'Exact-version rollback verification failed.' }
                Write-Event 'rollback' 'ok' @{ exitCode = $process.ExitCode; version = $restored[0].DisplayVersion }
            }
        }
    }
}
