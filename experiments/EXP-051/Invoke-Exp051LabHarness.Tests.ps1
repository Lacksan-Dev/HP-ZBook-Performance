$harness=Join-Path $PSScriptRoot 'Invoke-Exp051LabHarness.ps1'
Describe 'EXP-051 reboot-aware lab harness contract' {
 BeforeAll {$text=Get-Content -LiteralPath $harness -Raw}
 It 'retains Experimental evidence state' {Test-Path $harness|Should -BeTrue;$text|Should -Match 'EXP-051';$text|Should -Match 'needs-evidence';$text|Should -Not -Match 'status:stable|Stable='}
 It 'uses the Logi Bolt provider as sole treatment surface' {$text|Should -Match 'LogiBoltDemandLaunch.ps1';$text|Should -Match 'Provider Apply';$text|Should -Match 'Provider Rollback'}
 ...
 It 'retains Logi Bolt functionality as physical evidence' {foreach($t in 'manualLogiBoltLaunch','receiverAndBluetoothReadiness','pairingStateReadiness','protectedApplicationReadiness'){$text|Should -Match $t}}
