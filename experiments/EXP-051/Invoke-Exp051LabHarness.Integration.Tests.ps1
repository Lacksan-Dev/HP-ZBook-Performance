...
$provider=Join-Path $PSScriptRoot '..\..\controller\providers\LogiBoltDemandLaunch.ps1'
Describe 'EXP-051 lab harness integration' -Tag 'WindowsIntegration' {
 ...
 It 'exposes WhatIf start without creating continuation state' -Skip:($env:RUN_LACKSAN_WINDOWS_INTEGRATION-ne'1'-or$env:OS-ne'Windows_NT') {
  $root=Join-Path $TestDrive 'exp051';New-Item -ItemType Directory -Path $root -Force|Out-Null
  try{& $harness -Action Start -RunsPerArm 1 -SampleSeconds 30 -EvidenceRoot $root -ProviderPath $provider -WhatIf|Out-Null}catch{}
  (Get-ScheduledTask -TaskName 'Lacksan-EXP-051-LabHarness' -ErrorAction SilentlyContinue)|Should -BeNullOrEmpty
