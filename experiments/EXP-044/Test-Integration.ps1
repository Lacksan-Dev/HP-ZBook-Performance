[CmdletBinding()]
param([string]$ScriptPath=(Join-Path $PSScriptRoot 'Invoke-LogiOptionsPlusRunCalibration.ps1'))
...
$required=@('SupportsShouldProcess=$true',"'Check','Capture','DryRun','Apply','Verify','VerifyReboot','Rollback'",'DoNotExpandEnvironmentNames','Test-LogiOptionsPlusIdentity','ConvertTo-Json -Compress','Rollback refused because the registration already exists')
...
[pscustomobject]@{Experiment='EXP-044';ParsePassed=$true;ContractTokens=$required.Count;ForbiddenOperationsFound=0;LiveChangesApplied=$false}
