...
function Write-ExpLog { param([string]$Event,[string]$Result,[hashtable]$Data=@{})
    $record=[ordered]@{timestampUtc=(Get-Date).ToUniversalTime().ToString('o');experiment='EXP-044';action=$Action;event=$Event;result=$Result;data=$Data}
...
function Test-LogiOptionsPlusIdentity([string]$Name,[string]$Command) {
    if(Test-ProtectedIdentity $Name $Command){return $false}
    $nameMatch=$Name -match '(?i)^(Logi Options\+|LogiOptionsPlus|Logitech Options\+|Logi Options Plus)$'
    $exeMatch=$Command -match '(?i)(^|[\\/" ])(?:logioptionsplus_agent|logioptionsplus|logioptionsplus_appbroker)\.exe(?:[" ]|$)'
    $launchMatch=$Command -match '(?i)(--background|--minimized|--startup|--tray|--silent|/background|/minimized|/startup|/tray|/silent)'
    $updaterOnly=$Command -match '(?i)updater|update\.exe|logioptionsplus_updater'
...
function Get-RunCandidates {
    ...
    $items=foreach($name in $key.GetValueNames()){$value=[string]$key.GetValue($name,$null,[Microsoft.Win32.RegistryValueOptions]::DoNotExpandEnvironmentNames);if(Test-LogiOptionsPlusIdentity $name $value){[pscustomobject]@{Path=$script:RunPath;Name=$name;Data=$value;Kind=$key.GetValueKind($name).ToString()}}}; return @($items)
}
function Save-State([object[]]$Candidates){
    if($Candidates.Count -gt 1){throw 'More than one eligible Logi Options+ Run registration was found.'}
    $state=[ordered]@{schemaVersion=1;experiment='EXP-044';capturedUtc=(Get-Date).ToUniversalTime().ToString('o');machine=$env:COMPUTERNAME;userSid=([System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value);entries=@($Candidates)}
...
function Read-State {
    ...
    if($state.schemaVersion -ne 1 -or $state.experiment -ne 'EXP-044' -or $state.machine -ne $env:COMPUTERNAME -or $state.userSid -ne $sid){throw 'State identity validation failed.'};return $state
}
function Remove-Candidates([object[]]$Candidates){foreach($entry in $Candidates){if(-not(Test-LogiOptionsPlusIdentity $entry.Name $entry.Data)){throw "Candidate identity changed: $($entry.Name)"};if($PSCmdlet.ShouldProcess("$($entry.Path)::$($entry.Name)",'Remove Logi Options+ startup registration')){Remove-ItemProperty -LiteralPath $entry.Path -Name $entry.Name -ErrorAction SilentlyContinue}}}
function Restore-State($State){foreach($entry in @($State.entries)){if(-not(Test-LogiOptionsPlusIdentity $entry.Name $entry.Data)){throw "Rollback state identity rejected: $($entry.Name)"};if(Get-ItemProperty -LiteralPath $entry.Path -Name $entry.Name -ErrorAction SilentlyContinue){throw "Rollback refused because the registration already exists: $($entry.Name)"};if($PSCmdlet.ShouldProcess("$($entry.Path)::$($entry.Name)",'Restore Logi Options+ startup registration')){if(-not(Test-Path $entry.Path)){New-Item -Path $entry.Path -Force|Out-Null};$key=Get-Item -LiteralPath $entry.Path;$key.SetValue($entry.Name,[string]$entry.Data,[Microsoft.Win32.RegistryValueKind]::$($entry.Kind))}}}
...
try {
    $support=Get-SupportState;Write-ExpLog 'support-detection' ($(if($support.Supported){'pass'}else{'unsupported'})) @{os=$support.OS;build=$support.Build;manufacturer=$support.Manufacturer;model=$support.Model};if(-not $support.Supported){throw 'EXP-044 supports HP systems running Windows 11 only.'}
    switch($Action){
