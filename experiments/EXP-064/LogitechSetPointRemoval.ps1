...
$experiment = 'EXP-064'
$productNamePattern = '^Logitech\s+SetPoint(?:\s+Software)?$'
...
function Test-EnterpriseManaged {
    ...
    [bool](@($signals | Where-Object { Test-Path -LiteralPath $_ }).Count)
}

function Get-SetPointProduct {
    @(
...
function Test-UnsafeUninstallCommand {
    param([string]$Command)
    [bool]($Command -match '(?i)pnputil|dism(?:\.exe)?\s+.*?/remove-driver|devcon|Remove-PnpDevice|Disable-PnpDevice|firmware|pair(?:ing)?')
}

function Get-Support {
    $os = Get-CimInstance Win32_OperatingSystem
    $products = Get-SetPointProduct
    ...
    $command = if ($products.Count -eq 1) {
        if ($products[0].QuietUninstallString) { $products[0].QuietUninstallString } else { $products[0].UninstallString }
    } else { $null }
    $unsafe = if ($command) { Test-UnsafeUninstallCommand $command } else { $false }
    [pscustomobject]@{
        Supported = ($os.Caption -match 'Windows 11' -and (Test-Administrator) -and $products.Count -eq 1 -and $installerExists -and -not $managed -and -not $unsafe)
        Elevated = Test-Administrator
        EnterpriseManaged = $managed
        UnsafeUninstallCommand = $unsafe
        OS = $os.Caption
...
function Get-State {
    ...
    [ordered]@{
        ...
        installer = [ordered]@{ path = $support.OfflineInstaller; sha256 = $support.InstallerSha256; arguments = $InstallerArguments }
        preservedFunctions = @('PnP devices','Driver-store packages','USB and HID drivers','Bluetooth state','Receiver pairing','Firmware state','Basic keyboard and mouse operation')
        protectedApplications = @('Omnissa','Windows App','Remote Desktop','Tailscale')
    }
}
...
function Read-State {
    ...
    if ($state.schemaVersion -ne 1 -or $state.experiment -ne $experiment -or $state.machine -ne $env:COMPUTERNAME) { throw 'State identity validation failed.' }
    $state
}
...
function Assert-Supported {
    ...
    if ($Support.ProductCount -ne 1) { throw 'Exactly one Logitech-published SetPoint uninstall product is required.' }
    if ($Support.UnsafeUninstallCommand) { throw 'Uninstall command appears to target drivers, firmware, or pairing; application refused.' }
...
function Get-UninstallInvocation {
    ...
    if (Test-UnsafeUninstallCommand $command) { throw 'Unsafe uninstall command refused.' }
    ...
    if ($command -match '^\s*"([^"]+)"\s*(.*)$') { return [pscustomobject]@{ FilePath = $Matches[1]; Arguments = $Matches[2] } }
    $parts = $command -split '\s+',2
    [pscustomobject]@{ FilePath = $parts[0]; Arguments = if ($parts.Count -gt 1) { $parts[1] } else { '' } }
}

function Test-Removed { (Get-SetPointProduct).Count -eq 0 }
...
try {
    ...
    switch ($Action) {
        'Check' { $support }
        'Capture' { Assert-Supported $support; $state = Save-State; Write-Event 'capture' 'ok' $state; $state }
        'DryRun' {
            Assert-Supported $support
            $preview = [ordered]@{ remove = $support.Product.DisplayName; version = $support.Product.DisplayVersion; preserveDriversAndPairing = $true; rollbackInstallerSha256 = $support.InstallerSha256 }
        ...
        'Apply' {
            ...
            if ($support.Product.DisplayVersion -ne $state.product.displayVersion -or $support.Product.PSChildName -ne $state.product.productCode) { throw 'Installed-product identity drift detected.' }
