...
Supported operations and relevant states are:
...
Microsoft documentation used by the repository's packaged StartupTask decisions:
...
A production mutation provider remains gated because the repository requires exact rollback and EXP-154 has no physical state-transition proof for the selected EXP-143 candidate.

The supported API can disable an eligible task, while exact programmatic restoration depends on the captured runtime state and Windows ownership semantics. A production provider may proceed only after the exact target package/task combination demonstrates that an initially `Enabled` task can be disabled and restored to exactly `Enabled` through supported APIs in a documented repeatable workflow.
...
The physical probe must consume one exact EXP-143 normalized `priority-target` record and bind evidence to:

- the EXP-143 inventory artifact SHA-256;
- package full name and family name;
...
- registration source;
- machine identity, user SID, Windows build, boot identity, and management state in machine-local raw evidence only;
- normalized pre-change startup inventory and SHA-256.

Selection must refuse ambiguous candidates, duplicate product/mechanism coverage, missing package/task identity, unresolved executable identity where the manifest requires one, unsigned or identity-mismatched targets, management ownership, and every protected identity declared by EXP-154.

## Required physical state-transition probe

Run the probe only through the repository-authorized portfolio validation path on the target HP Windows 11 lab profile with the exact selected EXP-143 candidate:

1. Capture the complete EXP-143 selection binding and normalized packaged-StartupTask inventory before any state transition.
...
6. Verify the selected task reaches `Disabled` while every unrelated StartupTask remains equivalent in the normalized snapshot.
...
10. Reboot again and verify restored persistence plus equality of the normalized packaged-StartupTask inventory except declared volatile metadata.
...
14. Preserve every favorable, adverse, failed, rejected, and inconclusive transition as raw machine-local evidence.

## Provider acceptance contract after the probe passes

Only after exact restore is demonstrated may an EXP-154 mutation provider merge. It must implement:
...
- structured JSONL logging with experiment ID, timestamp, action, before state, after state, result, refusal reason, and failure detail, while raw machine/user identity remains machine-local;
