...
$experiment = 'EXP-060'
$productNamePattern = '^Logi(?:tech)? Bolt$'
...
function Get-State {
    ...
    [ordered]@{
        ...
        preservedFunctions = @('PnP devices','Driver-store packages','Bolt receiver pairing','Bluetooth pairing','Firmware state','Basic keyboard and mouse input')
        expectedTradeoffs = @('Bolt pairing management UI','Device-management UI','Future pairing changes without reinstall')
    }
}
...
function Assert-Supported {
    ...
    if ($Support.ProductCount -ne 1) { throw 'Exactly one Logitech-published Logi Bolt uninstall product is required.' }
...
function Test-Removed { return (Get-LogiBoltProduct).Count -eq 0 }
