BeforeAll {
    $scriptPath=Join-Path $PSScriptRoot '..\Invoke-EdgeEfficiencyModePolicy.ps1'
    $source=Get-Content -LiteralPath $scriptPath -Raw
}
Describe 'EXP-041 engineering contract' {
    It 'parses as PowerShell' {
        $tokens=$null;$errors=$null
        [void][System.Management.Automation.Language.Parser]::ParseFile($scriptPath,[ref]$tokens,[ref]$errors)
        $errors.Count|Should -Be 0
    }
    It 'uses only the recommended EfficiencyModeEnabled policy' {
        $source|Should -Match 'Recommended'
        $source|Should -Match 'EfficiencyModeEnabled'
        $source|Should -Match 'PropertyType DWord -Value 1'
    }
    It 'requires Edge 106 or later' {$source|Should -Match 'version.Major -ge 106'}
