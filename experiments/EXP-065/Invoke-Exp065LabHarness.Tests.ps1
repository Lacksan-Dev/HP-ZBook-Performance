Describe 'EXP-065 lab harness contract' {
  ...
  It 'propagates automatic reboot authorization to every logon continuation' {$text | Should -Match '\$automaticRebootArgument=if\(\$AllowAutomaticReboot\)';$text | Should -Match "' -AllowAutomaticReboot'";$text | Should -Match '\$ControllerPath`"\$automaticRebootArgument'}
