...
function Get-Candidate {
  ...
  $gs=Get-Service -Name $ServiceName -ErrorAction SilentlyContinue;$deps=@();$dependents=@();if($gs){$deps=@($gs.ServicesDependedOn|ForEach-Object {[string]$_.Name}|Sort-Object);$dependents=@($gs.DependentServices|ForEach-Object {[string]$_.Name}|Sort-Object)}
