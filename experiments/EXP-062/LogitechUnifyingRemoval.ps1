...
$experiment = 'EXP-062'
$productNamePattern = '^(Logitech\s+Unifying\s+Software|Logitech\s+Unifying\s+Receiver\s+Software)$'
...
function Get-LogitechUnifyingProduct {
...
function Test-UnsafeUninstallCommand {
    ...
    return [bool]($Command -match '(?i)pnputil|dism(?:\.exe)?\s+.*?/remove-driver|devcon|Remove-PnpDevice|Disable-PnpDevice|firmware|pair(?:ing)?')
}

function Get-Support {
    $os = Get-CimInstance Win32_OperatingSystem
    $products = Get-LogitechUnifyingProduct
    ...
    $unsafeCommand = if ($products.Count -eq 1) {
        $command = if ($products[0].QuietUninstallString) { $products[0].QuietUninstallString } else { $products[0].UninstallString }
        Test-UnsafeUninstallCommand $command
    } else { $false }
    [pscustomobject]@{
        Supported = ($os.Caption -match 'Windows 11' -and (Test-Administrator) -and $products.Count -eq 1 -and $installerExists -and -not $managed -and -not $unsafeCommand)
        Elevated = Test-Administrator
        EnterpriseManaged = $managed
        UnsafeUninstallCommand = $unsafeCommand
        OS = $os.Caption
...
function Get-State {
    ...
    [ordered]@{
        ...
        preservedFunctions = @('Unifying receiver','Existing receiver pairing','PnP devices','Driver-store packages','USB and HID drivers','Firmware state','Basic keyboard and mouse operation')
        expectedTradeoffs = @('Pairing or re-pairing through Logitech Unifying Software','Unifying management UI','Vendor update checks without reinstall')
    }
}
...
function Assert-Supported {
    ...
    if ($Support.ProductCount -ne 1) { throw 'Exactly one Logitech-published Unifying Software uninstall product is required.' }
...
function Test-Removed { return (Get-LogitechUnifyingProduct).Count -eq 0 }
...
try {
    ...
    switch ($Action) {
        ...
        'DryRun' {
            Assert-Supported $support
            $preview = [ordered]@{ remove = $support.Product.DisplayName; version = $support.Product.DisplayVersion; preserveReceiverPairingAndDrivers = $true; rollbackInstallerSha256 = $support.InstallerSha256 }
        ...
        'Rollback' {
            ...
            if ($PSCmdlet.ShouldProcess($state.product.displayName,'Reinstall exact captured product')) {
                ...
                $restored = Get-LogitechUnifyingProduct
