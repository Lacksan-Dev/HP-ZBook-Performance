...
param(
    ...
    [string]$ProviderPath="$PSScriptRoot\..\..\controller\providers\EdgeDisableShoppingAssistant.ps1"
)
Set-StrictMode -Version Latest
$ErrorActionPreference='Stop'
if($Unattended){$ConfirmPreference='None'}
$TaskName='Lacksan-EXP-156-LabHarness'
...
function WriteJson($p,$v){$v|ConvertTo-Json -Depth 24|Set-Content -LiteralPath $p -Encoding UTF8}
...
function Log($d,$event,$data){[ordered]@{schema=1;utc=(Get-Date).ToUniversalTime().ToString('o');experiment='EXP-156';event=$event;computer=$env:COMPUTERNAME;data=$data}|ConvertTo-Json -Compress -Depth 16|Add-Content (Join-Path $d 'lab-events.jsonl') -Encoding UTF8}
function Provider($a,$d){$p=Paths $d;& $ProviderPath -Action $a -StatePath $p.Provider -LogPath $p.Log -ProfileFixturePath $ProfileFixturePath -Confirm:$false}
function Protected{
    [pscustomobject]@{
        services=@('WinDefend','mpssvc','wuauserv','UsoSvc','BITS','edgeupdate','edgeupdatem','TermService','Tailscale'|ForEach-Object{$s=Get-Service $_ -ErrorAction SilentlyContinue;if($s){[pscustomobject]@{name=$_.ToString();status=$s.Status.ToString();startType=$s.StartType.ToString()}}})
...
function EdgeProcesses{@(Get-Process msedge -ErrorAction SilentlyContinue|Select-Object Id,ProcessName,StartTime,CPU,WorkingSet64,PrivateMemorySize64,Path)}
function Collect($phase,$ordinal,$d){
    $boot=Boot;$before=EdgeProcesses;$pb=Protected;$self0=(Get-Process -Id $PID).TotalProcessorTime.TotalMilliseconds;$samples=@();$n=[math]::Ceiling($SampleSeconds/[double]$SampleIntervalSeconds)
    for($i=0;$i-lt$n;$i++){$samples+=SampleSystem;if($i-lt$n-1){Start-Sleep $SampleIntervalSeconds}}
    $after=EdgeProcesses;$self1=(Get-Process -Id $PID).TotalProcessorTime.TotalMilliseconds;$explorer=Get-Process explorer -ErrorAction SilentlyContinue|Sort-Object StartTime|Select-Object -First 1
    [ordered]@{
        schema=1;experiment='EXP-156';phase=$phase;ordinal=$ordinal;computer=$env:COMPUTERNAME;bootUtc=$boot.ToString('o');capturedUtc=(Get-Date).ToUniversalTime().ToString('o')
        desktop=[ordered]@{explorerPresent=[bool]$explorer;bootToExplorerStartMs=if($explorer){($explorer.StartTime.ToUniversalTime()-$boot).TotalMilliseconds}else{$null}}
        system=[ordered]@{cpuMedianPercent=Median @($samples.cpu);cpuMadPercent=Mad @($samples.cpu);diskMedianBytesPerSec=Median @($samples.disk);diskMadBytesPerSec=Mad @($samples.disk)}
        edge=[ordered]@{before=$before;after=$after;processCountBefore=$before.Count;processCountAfter=$after.Count;shoppingRequestAttribution='needs-evidence';networkBytes='needs-evidence';gpuActivity='needs-evidence'}
        protected=[ordered]@{before=$pb;after=Protected}
        functional=[ordered]@{retailFixtureNavigation='needs-evidence';shoppingAssistantEligibilityOrAbsence='needs-evidence';autofillAndPaymentsRegression='needs-evidence';controlledNavigationReadiness='needs-evidence'}
        instrumentation=[ordered]@{collectorCpuDeltaMs=$self1-$self0;sampleCount=$samples.Count}
    }
}
function Summary($d){
    $r=@(Get-ChildItem $d -Filter 'run-*.json' -File|Sort-Object Name|ForEach-Object{ReadJson $_.FullName});$g=[ordered]@{}
    foreach($phase in 'Baseline','Treatment'){$x=@($r|Where-Object{$_.phase-eq$phase});$g[$phase]=[ordered]@{runs=$x.Count;bootToExplorerStartMs=[ordered]@{median=Median @($x.desktop.bootToExplorerStartMs);mad=Mad @($x.desktop.bootToExplorerStartMs)};cpuMedianPercent=[ordered]@{median=Median @($x.system.cpuMedianPercent);mad=Mad @($x.system.cpuMedianPercent)};diskMedianBytesPerSec=[ordered]@{median=Median @($x.system.diskMedianBytesPerSec);mad=Mad @($x.system.diskMedianBytesPerSec)};edgeProcessCountAfter=[ordered]@{median=Median @($x.edge.processCountAfter);mad=Mad @($x.edge.processCountAfter)}}}
    $s=[ordered]@{schema=1;experiment='EXP-156';generatedUtc=(Get-Date).ToUniversalTime().ToString('o');groups=$g;classification=if($g.Baseline.runs-lt$RunsPerArm-or$g.Treatment.runs-lt$RunsPerArm){'inconclusive'}else{'needs-functional-evidence'};functionalEvidence='needs-evidence';limitations=@('Explorer start is a usable-desktop proxy','Retail navigation, shopping-feature behavior, request/network attribution, payment/autofill checks, and controlled-navigation readiness require physical observation')};WriteJson (Paths $d).Summary $s;$s
}
function Task{Get-ScheduledTask -TaskName $TaskName -ErrorAction SilentlyContinue}
function RegisterTask($d){
    if(Task){throw 'Continuation task already exists'};$exe=(Get-Command powershell.exe).Source;$arg="-NoProfile -NonInteractive -ExecutionPolicy Bypass -File `"$PSCommandPath`" -Action Continue -Unattended -RunDirectory `"$d`" -EvidenceRoot `"$EvidenceRoot`" -ProfileFixturePath `"$ProfileFixturePath`" -ProviderPath `"$ProviderPath`"";if($AllowAutomaticReboot){$arg+=' -AllowAutomaticReboot'}
...
if(!(Test-Path -LiteralPath $ProviderPath)){throw 'EXP-156 provider missing'}
if($Action-ne'Status'-and[string]::IsNullOrWhiteSpace($ProfileFixturePath)){throw 'ProfileFixturePath is required for EXP-156 validation'}

switch($Action){
    'Start'{Ensure $EvidenceRoot;if(Test-Path $ActivePointer){throw 'Active EXP-156 run exists'};$check=Provider 'Check' $EvidenceRoot;if(!$check.Support.Supported){throw 'EXP-156 provider support gate refused the current machine or fixture'};if($WhatIfPreference){return $check};$d=Join-Path $EvidenceRoot (Get-Date).ToUniversalTime().ToString('yyyyMMdd-HHmmss');Ensure $d;$p=Paths $d;Provider 'Capture' $d|Out-Null;Provider 'DryRun' $d|Out-Null;WriteJson $p.Harness ([ordered]@{schema=1;phase='Baseline';completedRuns=0;runsPerArm=$RunsPerArm;lastBootUtc=$null;profileFixturePath=(Resolve-Path -LiteralPath $ProfileFixturePath).Path});WriteJson $ActivePointer @{runDirectory=$d};RegisterTask $d;Log $d 'started' @{runsPerArm=$RunsPerArm;sampleSeconds=$SampleSeconds;dryRun='pass';automaticReboot=[bool]$AllowAutomaticReboot};Reboot $d}
    ...
    'Status'{if(Test-Path $ActivePointer){$d=ActiveDir;ReadJson (Paths $d).Harness}else{[pscustomobject]@{active=$false;experiment='EXP-156'}}}
