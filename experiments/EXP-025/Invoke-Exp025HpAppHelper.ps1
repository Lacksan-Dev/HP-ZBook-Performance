...
function Get-Candidate {
  ...
  if($svc.PathName -notmatch '(?i)HP.*AppHelper|AppHelper.*Cap'){throw 'Service executable identity refused'}
  if($svc.PathName -match '(?i)Defender|SecurityHealth|BitLocker|Credential|Tailscale|Omnissa|RemoteDesktop|WindowsApp'){throw 'Protected identity refused'}
  $svc
}
...
function Read-State {
  ...
  if($s.schema -ne 1 -or $s.experiment -ne 'EXP-025' -or $s.computer -ne $env:COMPUTERNAME -or $s.service -ne $ServiceName){throw 'State identity refused'}
...
try {
  ...
  switch($Action){
    'Check' {
      Write-Event 'supported' @{service=$svc.Name;path=$svc.PathName;startMode=$svc.StartMode;state=$svc.State}
      $svc|Select-Object Name,DisplayName,PathName,StartMode,State
    }
    'Capture' { Save-State $svc|Out-Null }
    'DryRun' {
      $state=if(Test-Path -LiteralPath $StatePath){Read-State}else{Save-State $svc}
      $plan=[ordered]@{supported=$true;service=$svc.Name;path=$svc.PathName;currentStartMode=$svc.StartMode;targetStartMode='Manual';currentState=$svc.State;statePath=$StatePath;wouldChange=($svc.StartMode -ne 'Manual')}
      Write-Event 'dry-run' $plan
      [pscustomobject]$plan
    }
    'Apply' {
    ...
    'Verify' {
      $after=Get-Candidate
      $ok=($after.StartMode -eq 'Manual')
      Write-Event 'verified' @{manual=$ok;state=$after.State;path=$after.PathName}
      if(-not $ok){throw 'Verification failed'}
    }
    'VerifyReboot' {
      $after=Get-Candidate
      $ok=($after.StartMode -eq 'Manual')
      Write-Event 'reboot-verified' @{manual=$ok;state=$after.State;path=$after.PathName}
  ...
  Write-Event 'failure' @{type=$_.Exception.GetType().FullName;message=$_.Exception.Message}
  throw
}
