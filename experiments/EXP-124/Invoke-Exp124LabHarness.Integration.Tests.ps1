$harness=Join-Path $PSScriptRoot 'Invoke-Exp124LabHarness.ps1'
Describe 'EXP-124 lab harness zero-mutation integration' -Tag 'WindowsIntegration' {
    BeforeAll {
        ...
        function Snapshot-State {
            ...
            $security=Get-Service WinDefend,mpssvc,wuauserv,UsoSvc,BITS,ClickToRunSvc,Tailscale,TermService -ErrorAction SilentlyContinue|Sort-Object Name|Select-Object Name,Status,StartType
    ...
    It 'changes no Windows configuration during Start WhatIf' {
        ...
        $root=Join-Path $TestDrive 'exp124-harness'
        ...
        Get-ScheduledTask -TaskName 'Lacksan-EXP-124-LabHarness' -ErrorAction SilentlyContinue|Should -BeNullOrEmpty
    }
}
