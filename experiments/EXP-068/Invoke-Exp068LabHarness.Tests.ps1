...
$script=Join-Path $here 'Invoke-Exp068LabHarness.ps1'
Describe 'EXP-068 reboot validation harness contract' {
 ...
 It 'targets HPAudioAnalytics only' {$text|Should -Match "\$ServiceName='HPAudioAnalytics'"}
 ...
 It 'captures audio and hotkey evidence slots' {$text|Should -Match "audioPlayback='needs-evidence'";$text|Should -Match "hpHotkeys='needs-evidence'"}
