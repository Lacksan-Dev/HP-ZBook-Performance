...
function Write-Event([string]$Event,[hashtable]$Data){
  $row=[ordered]@{utc=(Get-Date).ToUniversalTime().ToString('o');experiment='EXP-024';action=$Action;event=$Event;computer=$env:COMPUTERNAME;userSid=([System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value);data=$Data}
...
function Get-Candidate {
  ...
  $gs=Get-Service -Name $ServiceName;$deps=@($gs.ServicesDependedOn|ForEach-Object Name);$dependents=@($gs.DependentServices|ForEach-Object Name);foreach($name in @($deps+$dependents)){if($name -match '(?i)WinDefend|mpssvc|bfe|wuauserv|bits|cryptsvc|Tailscale|Omnissa|TermService'){throw "Protected dependency refused: $name"}}
  [pscustomobject]@{Service=$svc;Executable=$exe;Version=$version.ToString();Hash=$hash;SignatureSubject=$sig.SignerCertificate.Subject;DelayedAutoStart=(Get-DelayedAutoStart);Dependencies=$deps;Dependents=$dependents;WindowsBuild=$os.BuildNumber}
}
function Save-State($candidate){$svc=$candidate.Service;$state=[ordered]@{schema=2;computer=$env:COMPUTERNAME;service=$svc.Name;displayName=$svc.DisplayName;path=$svc.PathName;executable=$candidate.Executable;executableVersion=$candidate.Version;executableHash=$candidate.Hash;signatureSubject=$candidate.SignatureSubject;startMode=$svc.StartMode;delayedAutoStart=$candidate.DelayedAutoStart;state=$svc.State;serviceAccount=$svc.StartName;dependencies=@($candidate.Dependencies);dependents=@($candidate.Dependents);windowsBuild=$candidate.WindowsBuild;capturedUtc=(Get-Date).ToUniversalTime().ToString('o')};$state|ConvertTo-Json -Depth 6|Set-Content -LiteralPath $StatePath -Encoding UTF8;Write-Event 'state-captured' @{service=$svc.Name;startMode=$svc.StartMode;delayedAutoStart=$candidate.DelayedAutoStart;state=$svc.State;version=$candidate.Version;hash=$candidate.Hash};$state}
function Read-State {if(-not(Test-Path -LiteralPath $StatePath)){throw 'State file missing'};$s=Get-Content -LiteralPath $StatePath -Raw|ConvertFrom-Json;if($s.schema -ne 2 -or $s.computer -ne $env:COMPUTERNAME -or $s.service -ne $ServiceName){throw 'State identity refused'};$s}
function Set-Mode([string]$Mode,[int]$DelayedAutoStart=0){$map=@{Auto='Automatic';Automatic='Automatic';Manual='Manual';Disabled='Disabled'};if(-not $map.ContainsKey($Mode)){throw "Unsupported startup mode: $Mode"};Set-Service -Name $ServiceName -StartupType $map[$Mode];if($map[$Mode] -eq 'Automatic'){Set-ItemProperty -LiteralPath $ServiceKey -Name DelayedAutoStart -Type DWord -Value $DelayedAutoStart}else{Set-ItemProperty -LiteralPath $ServiceKey -Name DelayedAutoStart -Type DWord -Value 0}}
...
switch($Action){
 ...
 'Verify' {$now=Get-Candidate;$ok=($now.Service.StartMode -eq 'Manual' -and $now.DelayedAutoStart -eq 0);Write-Event 'verified' @{manual=$ok;version=$now.Version;hash=$now.Hash};if(-not $ok){throw 'Verification failed'}}
 'VerifyReboot' {$now=Get-Candidate;$ok=($now.Service.StartMode -eq 'Manual' -and $now.DelayedAutoStart -eq 0);Write-Event 'reboot-verified' @{manual=$ok;state=$now.Service.State;version=$now.Version;hash=$now.Hash};if(-not $ok){throw 'Reboot persistence failed'}}
 'Rollback' {
   ...
   $after=Get-Candidate;if($after.Service.StartMode -ne $s.startMode -or $after.DelayedAutoStart -ne [int]$s.delayedAutoStart){throw 'Rollback verification failed'};Write-Event 'rolled-back' @{startMode=$after.Service.StartMode;delayedAutoStart=$after.DelayedAutoStart;state=$after.Service.State;version=$after.Version;hash=$after.Hash}
 }
}
