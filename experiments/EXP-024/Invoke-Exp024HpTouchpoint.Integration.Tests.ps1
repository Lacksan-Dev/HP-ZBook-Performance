...
Describe 'EXP-024 controller zero-mutation integration' -Tag 'WindowsIntegration' {
  BeforeAll {
    ...
    function Snapshot-ProductionState {
      ...
      if($svc){$target=$svc|Select-Object Name,State,StartMode,StartName,PathName}
      ...
      [ordered]@{Target=$target;Registry=$reg;Protected=$protected}|ConvertTo-Json -Compress -Depth 12
    }
  }
