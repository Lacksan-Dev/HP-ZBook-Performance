...
$experiment = 'EXP-061'
$productNamePattern = '^(Logitech\s+G\s+HUB|Logitech\s+G\s+Hub|G\s+HUB)$'
$publisherPattern = '^Logitech(?:\s|$)'
$uninstallRoots = @(
...
function Get-LogitechGHubProduct {
...
function Get-Support {
    ...
    $products = Get-LogitechGHubProduct
...
function Get-State {
    ...
    [ordered]@{
        ...
        preservedFunctions = @('PnP devices','Driver-store packages','Receiver pairing','Bluetooth pairing','Firmware state','Onboard device profiles','Basic keyboard, mouse, headset, microphone, speaker, and camera operation')
        expectedTradeoffs = @('G Hub device-management UI','Software profile switching','Macro editing','Lighting configuration','Update checks without reinstall')
    }
}
...
function Assert-Supported {
    ...
    if ($Support.ProductCount -ne 1) { throw 'Exactly one Logitech-published G Hub uninstall product is required.' }
...
function Test-Removed { return (Get-LogitechGHubProduct).Count -eq 0 }
...
try {
    ...
    switch ($Action) {
        ...
        'DryRun' {
            Assert-Supported $support
            $preview = [ordered]@{ remove = $support.Product.DisplayName; version = $support.Product.DisplayVersion; preserveDriversPairingFirmwareAndOnboardProfiles = $true; rollbackInstallerSha256 = $support.InstallerSha256 }
        ...
        'Rollback' {
            ...
            if ($PSCmdlet.ShouldProcess($state.product.displayName,'Reinstall exact captured product')) {
                ...
                $restored = Get-LogitechGHubProduct
