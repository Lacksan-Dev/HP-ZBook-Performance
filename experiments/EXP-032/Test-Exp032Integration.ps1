...
$probeRoot=Join-Path ([System.IO.Path]::GetTempPath()) ('EXP-032-'+[guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $probeRoot -Force|Out-Null
try {
  $statePath=Join-Path $probeRoot 'state.json';$logPath=Join-Path $probeRoot 'events.jsonl'
  try {
  ...
  if(Test-Path -LiteralPath $logPath){
    foreach($line in Get-Content -LiteralPath $logPath){$event=$line|ConvertFrom-Json;if($event.experiment -ne 'EXP-032'){throw 'Structured log experiment identity mismatch'};if(-not $event.utc -or -not $event.event){throw 'Structured log missing required fields'}}
  }
  'EXP-032 non-destructive integration checks passed.'
}
