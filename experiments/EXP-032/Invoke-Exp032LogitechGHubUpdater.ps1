...
function Write-Event([string]$Event,[hashtable]$Data){
  ...
  $row=[ordered]@{utc=(Get-Date).ToUniversalTime().ToString('o');experiment='EXP-032';action=$Action;event=$Event;computer=$env:COMPUTERNAME;data=$Data}
...
function Get-Candidate {
  ...
  if($svc.DisplayName -notmatch '(?i)Logitech.*G HUB.*Updater|G HUB.*Updater'){throw 'Service display-name identity refused'}
  if($svc.PathName -notmatch '(?i)logitech|lghub' -or $svc.PathName -notmatch '(?i)updater'){throw 'Service executable identity refused'}
...
function Save-State($svc){
  ...
  $state=[ordered]@{schema=1;experiment='EXP-032';computer=$env:COMPUTERNAME;service=$svc.Name;displayName=$svc.DisplayName;path=$svc.PathName;startMode=$svc.StartMode;delayedAutoStart=(Get-DelayedAutoStart);state=$svc.State;capturedUtc=(Get-Date).ToUniversalTime().ToString('o')}
...
function Read-State {
  ...
  if($s.schema -ne 1 -or $s.experiment -ne 'EXP-032' -or $s.computer -ne $env:COMPUTERNAME -or $s.service -ne $ServiceName){throw 'State identity refused'}
