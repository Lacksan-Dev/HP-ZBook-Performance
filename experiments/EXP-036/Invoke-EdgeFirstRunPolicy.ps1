...
function Get-ManagementState {
    $joined=$false;$signals=@()
    try {$text=(& dsregcmd.exe /status 2>$null)-join "`n";foreach($name in @('AzureAdJoined','DomainJoined','WorkplaceJoined')){if($text -match "(?im)^\s*$name\s*:\s*YES\s*$"){$joined=$true;$signals+=$name}}} catch {}
    [pscustomobject]@{Managed=$joined;Signals=$signals}
}
function Get-SupportState {
    $os=Get-CimInstance Win32_OperatingSystem;$computer=Get-CimInstance Win32_ComputerSystem;$edge=Get-EdgePath
    $version=if($edge){[version](Get-Item -LiteralPath $edge).VersionInfo.ProductVersion}else{$null};$management=Get-ManagementState
    $admin=([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
    $supported=([version]$os.Version).Major -ge 10 -and $os.Caption -match 'Windows 11' -and $computer.Manufacturer -match 'HP|Hewlett-Packard' -and $edge -and $version.Major -ge 80
    [pscustomobject]@{Supported=$supported;OS=$os.Caption;Build=$os.BuildNumber;Manufacturer=$computer.Manufacturer;Model=$computer.Model;EdgePath=$edge;EdgeVersion=$version.ToString();Managed=$management.Managed;ManagementSignals=$management.Signals;Elevated=$admin}
}
function Get-PolicyState {
    $keyExists=Test-Path -LiteralPath $script:PolicyPath;$valueExists=$false;$data=$null;$kind=$null
    if($keyExists){$key=Get-Item -LiteralPath $script:PolicyPath;$valueExists=$key.GetValueNames() -contains $script:ValueName;if($valueExists){$data=$key.GetValue($script:ValueName,$null,[Microsoft.Win32.RegistryValueOptions]::DoNotExpandEnvironmentNames);$kind=$key.GetValueKind($script:ValueName).ToString()}}
    [pscustomobject]@{KeyExists=$keyExists;ValueExists=$valueExists;Data=$data;Kind=$kind}
}
function Save-State($support,$policy){
    $state=[ordered]@{schemaVersion=1;experiment='EXP-036';capturedUtc=(Get-Date).ToUniversalTime().ToString('o');machine=$env:COMPUTERNAME;policyPath=$script:PolicyPath;valueName=$script:ValueName;edgePath=$support.EdgePath;edgeVersion=$support.EdgeVersion;keyExists=$policy.KeyExists;valueExists=$policy.ValueExists;data=$policy.Data;kind=$policy.Kind}
    $state|ConvertTo-Json -Depth 8|Set-Content -LiteralPath $StatePath -Encoding UTF8;return $state
}
function Read-State {
    if(-not(Test-Path -LiteralPath $StatePath)){throw "State file missing: $StatePath"};$state=Get-Content -LiteralPath $StatePath -Raw|ConvertFrom-Json
    if($state.schemaVersion -ne 1 -or $state.experiment -ne 'EXP-036' -or $state.machine -ne $env:COMPUTERNAME -or $state.policyPath -ne $script:PolicyPath -or $state.valueName -ne $script:ValueName){throw 'State identity validation failed.'};return $state
}
...
function Restore-State($state){
    $current=Get-PolicyState;if($state.valueExists){throw 'Captured state contains a pre-existing policy value; this experiment should never have applied.'}
    if($current.ValueExists -and -not($current.Kind -eq 'DWord' -and [int]$current.Data -eq 1)){throw 'Rollback refused because the policy value changed after application.'}
    if($current.ValueExists -and $PSCmdlet.ShouldProcess("$($script:PolicyPath)::$($script:ValueName)",'Remove experiment policy value')){Remove-ItemProperty -LiteralPath $script:PolicyPath -Name $script:ValueName -ErrorAction Stop}
...
try {
    $support=Get-SupportState;Write-ExpLog 'support-detection' ($(if($support.Supported){'pass'}else{'unsupported'})) @{os=$support.OS;build=$support.Build;manufacturer=$support.Manufacturer;model=$support.Model;edgeVersion=$support.EdgeVersion;managed=$support.Managed;elevated=$support.Elevated}
    ...
    switch($Action){
        ...
        'DryRun' {$p=Get-PolicyState;Assert-ApplySafe $support $p;Write-ExpLog 'dry-run' 'pass' @{wouldSet=1;restartRequired=$true};[pscustomobject]@{WouldSet="$($script:PolicyPath)::$($script:ValueName)";Data=1;RestartRequired=$true}}
        'Apply' {$p=Get-PolicyState;Assert-ApplySafe $support $p;if(-not(Test-Path -LiteralPath $StatePath)){Save-State $support $p|Out-Null}else{[void](Read-State)};Set-ExperimentPolicy;if(-not(Test-Applied)){throw 'Apply verification failed.'};Write-ExpLog 'apply' 'pass' @{value=1;browserRestartRequired=$true};[pscustomobject]@{Applied=$true;BrowserRestartRequired=$true}}
