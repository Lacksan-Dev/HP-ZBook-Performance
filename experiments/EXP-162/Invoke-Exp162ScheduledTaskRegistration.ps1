...
function Get-TaskIdentity {
    ...
    [pscustomobject][ordered]@{
        taskName = $TaskName; taskPath = $TaskPath; enabled = [bool]$task.Settings.Enabled
...
function Save-State($Identity,$Protected) {
    Ensure-Parent $StatePath
    [pscustomobject][ordered]@{
        schemaVersion = 1; experiment = $ExperimentId; capturedUtc = (Get-Date).ToUniversalTime().ToString('o')
        machine = $env:COMPUTERNAME; userSid = [Security.Principal.WindowsIdentity]::GetCurrent().User.Value
        selfManagedLab = [bool]$SelfManagedLab; original = $Identity; protected = $Protected
...
function Assert-ProtectedUnchanged($Captured) {
    $current = Get-ProtectedFingerprint | ConvertTo-Json -Depth 10 -Compress
...
try {
    switch ($Action) {
        'Check' {
            Assert-SafeContext
            $id = Get-TaskIdentity
            [pscustomobject]@{experiment=$ExperimentId;supported=$true;candidate=$id}
        }
        'Capture' {
            Assert-SafeContext
            $id = Get-TaskIdentity
            if (-not $id.enabled) { throw 'Candidate must be enabled at baseline.' }
            $protected = Get-ProtectedFingerprint
            Save-State $id $protected
        ...
        'Apply' {
            Assert-SafeContext
            $state = Load-State
            Assert-ProtectedUnchanged $state.protected
            $id = Get-TaskIdentity
        ...
        'Verify' {
            ...
            Write-Log Verify success 'Treatment state verified.' $state.original $id
            $id
        }
        'VerifyReboot' {
            $state = Load-State
            ...
            Write-Log VerifyReboot success 'Disabled state persisted across reboot.' $state.original $id
            $id
        }
        'Rollback' {
            Assert-SafeContext
            ...
            $after = Get-TaskIdentity
            if ($after.structuralHash -ne $state.original.structuralHash -or $after.enabled -ne [bool]$state.original.enabled) { throw 'Exact rollback verification failed.' }
            Write-Log Rollback success 'Captured enabled state restored with unchanged task structure.' $id $after
            $after
        }
    }
} catch {
