...
Describe 'EXP-053 reboot-aware lab harness contract' {
 BeforeAll {$text=Get-Content -LiteralPath $harness -Raw}
 It 'retains Experimental evidence state' {Test-Path $harness|Should -BeTrue;$text|Should -Match 'EXP-053';$text|Should -Match 'needs-evidence';$text|Should -Not -Match 'status:stable|Stable='}
 It 'uses the Logitech G Hub provider as sole treatment surface' {$text|Should -Match 'LogitechGHubDemandLaunch.ps1';$text|Should -Match 'Provider Apply';$text|Should -Match 'Provider Rollback'}
 ...
 It 'retains G Hub functionality as physical evidence' {foreach($t in 'manualGHubLaunch','deviceProfileReadiness','logitechHidAudioVideoReadiness','protectedApplicationReadiness'){$text|Should -Match $t}}
