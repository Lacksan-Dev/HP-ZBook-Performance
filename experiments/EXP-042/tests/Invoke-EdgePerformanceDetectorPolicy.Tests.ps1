...
Describe 'EXP-042 engineering contract' {
    It 'parses as PowerShell' {
        $tokens=$null;$errors=$null
        [void][System.Management.Automation.Language.Parser]::ParseFile($scriptPath,[ref]$tokens,[ref]$errors)
        $errors.Count|Should -Be 0
    }
    It 'uses only the recommended PerformanceDetectorEnabled policy' {
        $source|Should -Match 'Recommended'
        $source|Should -Match 'PerformanceDetectorEnabled'
        $source|Should -Match "TreatmentValue=0"
        $source|Should -Match "Set REG_DWORD 0"
    }
    It 'requires Edge 107 or later' {$source|Should -Match 'version.Major -ge 107'}
    ...
    It 'excludes destructive and protected-scope operations' {
        foreach($token in @('Remove-AppxPackage','Uninstall-Package','Set-Service','Stop-Service','Disable-ScheduledTask','Unregister-ScheduledTask','pnputil','devcon','Remove-Item -Recurse')){$source.Contains($token)|Should -BeFalse}
    }
    It 'verifies exact rollback and mutation refusal' {
        ...
        $source|Should -Match 'Captured state was ineligible'
    }
}
