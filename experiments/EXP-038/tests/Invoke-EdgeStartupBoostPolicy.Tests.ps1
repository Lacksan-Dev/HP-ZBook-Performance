BeforeAll{$scriptPath=Join-Path $PSScriptRoot '..\Invoke-EdgeStartupBoostPolicy.ps1';$source=Get-Content -LiteralPath $scriptPath -Raw}
Describe 'EXP-038 engineering contract'{
...
It 'uses the recommended StartupBoostEnabled policy only'{$source|Should -Match 'Recommended';$source|Should -Match 'StartupBoostEnabled';$source|Should -Match 'PropertyType DWord -Value 0'}
It 'requires Edge 88 or later'{$source|Should -Match 'v.Major -ge 88'}
