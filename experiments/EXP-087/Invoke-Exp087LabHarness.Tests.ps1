Describe 'EXP-087 lab harness contract' {
 ...
 It 'delegates treatment and exact rollback to the current hardened EXP-087 provider' {$text|Should -Match 'HPSupportSolutionsFrameworkManual.ps1';$text|Should -Not -Match 'HpSupportSolutionsFrameworkManualDemandStart.ps1';$text|Should -Match "Provider 'Apply'";$text|Should -Match "Provider 'Rollback'"}
 It 'uses the current provider interface without legacy acknowledgement parameters' {$text|Should -Match '& \$ProviderPath -Action \$a -StatePath \$p.Provider -LogPath \$p.Log -Confirm:\$false';$text|Should -Not -Match 'AcknowledgeHpWorkflowValidation'}
 ...
 It 'suppresses nested provider confirmation prompts without widening provider scope' {$text|Should -Match '-Confirm:\$false';$text|Should -Not -Match 'Set-Service'}
}
