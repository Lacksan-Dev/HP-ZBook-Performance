...
function Write-Event([string]$Event,[hashtable]$Data){
  ...
  $row=[ordered]@{utc=(Get-Date).ToUniversalTime().ToString('o');experiment='EXP-027';action=$Action;event=$Event;computer=$env:COMPUTERNAME;data=$Data}
...
function Get-Candidate {
  ...
  if($svc.PathName -notmatch '(?i)HP.*Sys.*Info|SysInfo.*Cap'){throw 'Service executable identity refused'}
...
function Save-State($svc){
  ...
  if($parent -and -not(Test-Path -LiteralPath $parent)){New-Item -ItemType Directory -Path $parent -Force|Out-Null}
  $state=[ordered]@{schema=1;experiment='EXP-027';computer=$env:COMPUTERNAME;service=$svc.Name;displayName=$svc.DisplayName;path=$svc.PathName;startMode=$svc.StartMode;state=$svc.State;capturedUtc=(Get-Date).ToUniversalTime().ToString('o')}
...
function Read-State {
  ...
  if($s.schema -ne 1 -or $s.experiment -ne 'EXP-027' -or $s.computer -ne $env:COMPUTERNAME -or $s.service -ne $ServiceName){throw 'State identity refused'}
