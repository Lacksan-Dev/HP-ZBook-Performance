...
Describe 'EXP-143 startup inventory classification contract' {
 ...
 It 'keeps inventory read only' {$text|Should -Not -Match 'Remove-ItemProperty|Remove-Item\s|Disable-ScheduledTask|Unregister-ScheduledTask|Remove-AppxPackage|Set-Service|Stop-Service|Disable-PnpDevice|pnputil|Set-Acl'}
 ...
 It 'captures shortcut working-directory evidence' {$text|Should -Match '\$shortcut\.WorkingDirectory';$text|Should -Match 'workingDirectory=\$workingDirectory'}
 It 'captures Startup-folder owner and SDDL through read-only ACL APIs' {$text|Should -Match 'Get-Acl -LiteralPath';$text|Should -Match '\$acl\.Owner';$text|Should -Match 'GetSecurityDescriptorSddlForm';$text|Should -Match 'owner=\$owner';$text|Should -Match 'sddl=\$sddl'}
 It 'marks unreadable ACL evidence explicitly' {$text|Should -Match "\$aclEvidenceStatus = 'needs-evidence'";$text|Should -Match 'aclEvidenceErrorType'}
 ...
 It 'uses schema version 6 deterministic evidence' {$text|Should -Match "schemaVersion=6";$text|Should -Match 'snapshotSha256'}
}
