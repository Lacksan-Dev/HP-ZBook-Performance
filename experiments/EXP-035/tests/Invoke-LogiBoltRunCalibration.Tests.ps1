BeforeAll {
    $scriptPath = Join-Path $PSScriptRoot '..\Invoke-LogiBoltRunCalibration.ps1'
    $source = Get-Content -LiteralPath $scriptPath -Raw
}
Describe 'EXP-035 engineering contract' {
    ...
    It 'uses bounded Logi Bolt identity checks' {
        $source | Should -Match 'Test-LogiBoltIdentity'
