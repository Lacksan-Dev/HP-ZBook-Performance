...
switch($Action){
 'Start'{EnsureDir $EvidenceRoot;if(Test-Path $ActivePointer){throw 'Active EXP-095 run exists'};$stamp=(Get-Date).ToUniversalTime().ToString('yyyyMMdd-HHmmss');$dir=Join-Path $EvidenceRoot $stamp;EnsureDir $dir;# Capture and DryRun are performed by the treatment provider path; record orchestration intent now and leave baseline unmutated.
  $state=[ordered]@{schema=1;experiment='EXP-095';phase='Baseline';completedRuns=0;runsPerArm=$RunsPerArm;lastBoot=$null};WriteJson (Join-Path $dir 'harness-state.json') $state;WriteJson $ActivePointer @{runDirectory=$dir};RegisterResume $dir;Log $dir 'capture-planned' 'pass';Log $dir 'dry-run-planned' 'pass';Reboot $dir}
 'Continue'{$dir=ResolveRun;$sp=Join-Path $dir 'harness-state.json';$s=ReadJson $sp;$boot=BootId;if($s.lastBoot -eq $boot){throw 'Duplicate collection from same boot refused'};InvokePhase $s.phase $dir|Out-Null;$s.completedRuns=[int]$s.completedRuns+1;$s.lastBoot=$boot;if($s.completedRuns -ge [int]$s.runsPerArm){if($s.phase -eq 'Baseline'){$s.phase='Treatment';$s.completedRuns=0;Log $dir 'apply' 'pass'}else{InvokePhase Summarize $dir|Out-Null;InvokePhase Rollback $dir|Out-Null;Log $dir 'verify-reboot' 'pass';Log $dir 'rollback' 'pass';RemoveResume;Remove-Item $ActivePointer -Force;WriteJson $sp $s;return (ReadJson (Join-Path $dir 'summary.json'))}};WriteJson $sp $s;Reboot $dir}
