...
Describe 'EXP-114 LogiTuneSigninTask provider contract' {
 ...
 It 'preserves application updater firmware driver and device scope'{foreach($t in 'PreserveApplication','PreserveUpdater','PreserveFirmware','PreserveDrivers','PreserveDevices','firmware','driver'){$text|Should -Match $t}}
 It 'requires observed reboot persistence and exact drift-aware rollback'{foreach($t in 'A later boot is required','Task-definition drift detected','Logi Tune executable or product identity drift detected','Protected-scope drift detected','Exact rollback verification failed','restoredExactOriginal'){$text|Should -Match [regex]::Escape($t)}}
