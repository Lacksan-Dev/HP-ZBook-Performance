Describe 'EXP-127 Microsoft365StartupFolder zero-mutation integration' -Tag 'Integration' {
    BeforeAll {
        ...
        $provider = Join-Path $PSScriptRoot '..\providers\Microsoft365StartupFolder.ps1'
        $script:state = Join-Path $TestDrive 'exp127-state.json'
        $script:log = Join-Path $TestDrive 'exp127-events.jsonl'
        function Snapshot-System {
            [ordered]@{
                ...
                OfficePackages = @(Get-AppxPackage -ErrorAction SilentlyContinue | Where-Object { $_.Name -match '(?i)office|outlook|microsoft365' } | Sort-Object PackageFullName | ForEach-Object { "$($_.PackageFullName)|$($_.Status)" })
