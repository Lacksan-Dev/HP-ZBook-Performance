Describe 'EdgeRequireHardwareAcceleration integration' -Tag 'WindowsIntegration' {
 BeforeAll{
  $script:providerPath=Join-Path $PSScriptRoot '..\providers\EdgeRequireHardwareAcceleration.ps1'
 ...
 It 'keeps policy startup service task driver and device state unchanged through read only paths'{
  ...
  $evidence=Join-Path $TestDrive 'exp078-gpu.json'
  @{experiment='EXP-078';controlledEvidence=$true;gpuHealthy=$true;hardwareAccelerationEffective=$false;edgeGpuEvidenceSha256='fixture';rendererClassification='software-fallback'}|ConvertTo-Json|Set-Content $evidence
  $before=Snapshot-State
  & $script:providerPath -Action Check -GraphicsEvidencePath $evidence|Out-Null
  (Snapshot-State)|Should -BeExactly $before
  try{& $script:providerPath -Action DryRun -GraphicsEvidencePath $evidence|Out-Null}catch{}
  (Snapshot-State)|Should -BeExactly $before
  try{& $script:providerPath -Action Apply -GraphicsEvidencePath $evidence -StatePath (Join-Path $TestDrive 'exp078-state.json') -WhatIf|Out-Null}catch{}
  (Snapshot-State)|Should -BeExactly $before
 }
}
