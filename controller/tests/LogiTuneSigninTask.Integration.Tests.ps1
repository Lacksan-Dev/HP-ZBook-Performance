Describe 'EXP-114 LogiTuneSigninTask zero-mutation integration' -Tag 'WindowsIntegration' {
 BeforeAll {
  $script:providerPath=Join-Path $PSScriptRoot '..\providers\LogiTuneSigninTask.ps1'
 ...
 It 'keeps task service driver device and protected-process state unchanged during Check Capture DryRun and Apply WhatIf' {
  ...
  & $script:providerPath -Action Check -LogPath $log|Out-Null;(Snapshot-State)|Should -BeExactly $before
  try{& $script:providerPath -Action Capture -StatePath $state -LogPath $log|Out-Null}catch{};(Snapshot-State)|Should -BeExactly $before
  if(Test-Path -LiteralPath $state){Remove-Item -LiteralPath $state -Force}
  try{& $script:providerPath -Action DryRun -StatePath $state -LogPath $log|Out-Null}catch{};(Snapshot-State)|Should -BeExactly $before
  try{& $script:providerPath -Action Apply -StatePath $state -LogPath $log -WhatIf|Out-Null}catch{};(Snapshot-State)|Should -BeExactly $before
 }
}
