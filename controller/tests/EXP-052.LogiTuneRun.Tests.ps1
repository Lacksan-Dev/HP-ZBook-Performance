$providerPath=Join-Path $PSScriptRoot '..\providers\LogiTuneRun.ps1'
$manifestPath=Join-Path $PSScriptRoot '..\lacksan.manifest.json'
Describe 'EXP-052 Logi Tune Run provider contract' {
    ...
    It 'requires exact Logi Tune executable and background identity' {foreach($t in 'Logi Tune','LogiTune','LogiTuneUI','logi-tune','--background','--startup','--tray','--silent'){$text|Should -Match ([regex]::Escape($t))}}
    It 'refuses updater pairing firmware driver and protected identities' {foreach($t in 'updater','pairing','firmware','driver','omnissa','tailscale','receiver','bluetooth','audio service','camera service'){$text|Should -Match $t}}
    ...
    It 'is registered with every protected scope' {$manifest.profiles.id|Should -Contain 'LogiTuneDemandLaunch';$entry=$manifest.providers|Where-Object id -eq 'logi-tune-run';$entry.mode|Should -Be 'Reversible';foreach($s in 'WindowsSecurity','WindowsUpdate','Recovery','EnterpriseManagement','DeviceCriticalDrivers','Omnissa','WindowsApp','RemoteDesktop','Tailscale'){$entry.protectedScopes|Should -Contain $s}}
}
