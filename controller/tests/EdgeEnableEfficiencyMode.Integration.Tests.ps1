Describe 'EdgeEnableEfficiencyMode integration' -Tag 'WindowsIntegration' {
 BeforeAll{
  $script:providerPath=Join-Path $PSScriptRoot '..\providers\EdgeEnableEfficiencyMode.ps1'
 ...
 It 'keeps policy startup service task driver and device state unchanged through read-only paths'{
  ...
  (Snapshot-State)|Should -BeExactly $before
  try{& $script:providerPath -Action Apply -StatePath (Join-Path $TestDrive 'exp074-efficiency-mode-state.json') -WhatIf|Out-Null}catch{}
  (Snapshot-State)|Should -BeExactly $before
 }
}
