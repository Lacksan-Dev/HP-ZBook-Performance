...
Describe 'EXP-070 HP Connection Recovery manual demand-start provider contract' {
 ...
 It 'targets exactly HPCommRecovery and Manual startup only' {$text|Should -Match "ServiceName='HPCommRecovery'";$text|Should -Match 'HPCommRecovery\.exe';$text|Should -Match 'Set-Service -Name \$ServiceName -StartupType Manual';$text|Should -Not -Match 'Get-Service \*|Set-Service .+\*'}
 ...
 It 'refuses active recovery activity' {$text|Should -Match 'ActiveRecoveryProcesses';$text|Should -Match 'active HP Connection Recovery process detected';$text|Should -Match 'Active recovery activity detected'}
 ...
 It 'contains no network driver package or security mutation commands' {$text|Should -Not -Match 'Disable-NetAdapter|Set-NetIP|New-NetRoute|Set-DnsClient|Set-NetFirewall|Disable-PnpDevice|pnputil|Remove-AppxPackage|Uninstall-Package|bcdedit|sc\.exe\s+delete'}
