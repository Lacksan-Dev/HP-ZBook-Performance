...
Describe 'EXP-146 zero-mutation Windows integration' -Tag 'WindowsIntegration' {
 BeforeAll {
  ...
  function Snapshot-Protected {
   Get-CimInstance Win32_Service -ErrorAction SilentlyContinue|Where-Object{$_.Name-in@('WinDefend','mpssvc','wuauserv','UsoSvc','BITS','TermService','Tailscale')}|Sort-Object Name|Select-Object Name,State,StartMode,PathName|ConvertTo-Json -Compress -Depth 5
  }
 }
 It 'Check and DryRun preserve Startup folders and protected services' -Skip:(-not$script:enabled) {
  $selection=$env:LACKSAN_EXP146_SELECTION;if([string]::IsNullOrWhiteSpace($selection)-or!(Test-Path $selection)){Set-ItResult -Skipped -Because 'Set LACKSAN_EXP146_SELECTION to a real EXP-143 selection artifact.';return}
  ...
  $check=& $provider -Action Check -SelectionPath $selection -StatePath $state -LogPath $log
  if($check.Supported){& $provider -Action DryRun -SelectionPath $selection -StatePath $state -LogPath $log|Out-Null}
 ...
 It 'Capture and Apply WhatIf preserve production state when eligible' -Skip:(-not$script:enabled) {
  ...
  $check=& $provider -Action Check -SelectionPath $selection -StatePath $state -LogPath $log;if(!$check.Supported){Set-ItResult -Skipped -Because ($check.Reasons-join'; ');return}
  & $provider -Action Capture -SelectionPath $selection -StatePath $state -LogPath $log|Out-Null;& $provider -Action Apply -SelectionPath $selection -StatePath $state -LogPath $log -WhatIf|Out-Null
 ...
 It 'contains no broad destructive operation' {
  $text=Get-Content -LiteralPath $provider -Raw;$text|Should -Not -Match 'Remove-AppxPackage|Uninstall-Package|Set-Service|Stop-Service|Remove-Service|Disable-PnpDevice|pnputil|Disable-ScheduledTask|Unregister-ScheduledTask|Set-MpPreference|Set-NetFirewall'
 }
}
