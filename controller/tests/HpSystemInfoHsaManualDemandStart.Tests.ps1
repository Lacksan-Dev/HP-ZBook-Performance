...
Describe 'EXP-065 HP System Info HSA manual demand-start provider contract' {
 ...
 It 'refuses management and holds protected security update and remote access configuration' {foreach($t in 'enterprise management ownership detected','CcmExec','Enrollments','OMADM','WinDefend','mpssvc','wuauserv','UsoSvc','BITS','TermService','Tailscale'){$text|Should -Match $t}}
 It 'separates reboot-varying protected runtime from protected configuration drift' {
  $text|Should -Match 'Configuration=@\(\$rows\|Select-Object Name,StartMode,PathName\)'
  $text|Should -Match 'Runtime=@\(\$rows\|Select-Object Name,State\)'
  $text|Should -Match 'ProtectedConfigFromState'
  $text|Should -Match 'protectedNow\.Configuration'
  ...
  $text|Should -Not -Match 'Select-Object Name,State,StartMode,PathName'
 }
 It 'supports legacy captured protected snapshots while refusing missing protected configuration' {
  $text|Should -Match "PSObject\.Properties\['Configuration'\]"
  $text|Should -Match "PSObject\.Properties\['Services'\]"
  $text|Should -Match 'Captured protected configuration is missing'
 }
 ...
 It 'implements drift-safe exact rollback including delayed-auto-start running state and protected configuration' {foreach($t in 'Rollback collision','delayed-auto-start drift','RegistryValueKind','Start-Service','Stop-Service','Exact rollback verification failed','Protected configuration changed during rollback','restoredExactOriginal'){$text|Should -Match $t}}
