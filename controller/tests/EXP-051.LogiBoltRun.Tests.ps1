$providerPath=Join-Path $PSScriptRoot '..\providers\LogiBoltRun.ps1'
$manifestPath=Join-Path $PSScriptRoot '..\lacksan.manifest.json'
Describe 'EXP-051 Logi Bolt Run provider contract' {
    ...
    It 'requires exact Logi Bolt executable and background identity' {foreach($t in 'Logi Bolt','LogiBolt','logi-bolt','--background','--startup','--tray','--silent'){$text|Should -Match ([regex]::Escape($t))}}
    It 'refuses updater pairing firmware and protected identities' {foreach($t in 'updater','pairing','firmware','omnissa','tailscale','receiver','bluetooth'){$text|Should -Match $t}}
    ...
    It 'is registered with every protected scope' {$manifest.profiles.id|Should -Contain 'LogiBoltDemandLaunch';$entry=$manifest.providers|Where-Object id -eq 'logi-bolt-run';$entry.mode|Should -Be 'Reversible';foreach($s in 'WindowsSecurity','WindowsUpdate','Recovery','EnterpriseManagement','DeviceCriticalDrivers','Omnissa','WindowsApp','RemoteDesktop','Tailscale'){$entry.protectedScopes|Should -Contain $s}}
}
