Describe 'EXP-135 issue 305 NonAllowlistedStartupTaskInventory research contract' {
 ...
 It 'offers read-only Check Capture and explicit DryRun actions'{$script:text|Should -Match "ValidateSet\('Check','Capture','DryRun'\)";foreach($t in 'Apply','Rollback','StartupApproved','Set-ItemProperty','Remove-ItemProperty','Register-ScheduledTask','Disable-ScheduledTask','Enable-ScheduledTask','Remove-AppxPackage'){$script:text|Should -Not -Match $t};$script:text|Should -Not -Match '\$[A-Za-z_][A-Za-z0-9_]*\.Disable\('; $script:text|Should -Not -Match '\$[A-Za-z_][A-Za-z0-9_]*\.RequestEnableAsync\('}
