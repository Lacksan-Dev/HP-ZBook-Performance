Describe 'EdgeDisableNetworkPrediction integration' -Tag 'WindowsIntegration' {
 BeforeAll{
  $script:providerPath=Join-Path $PSScriptRoot '..\providers\EdgeDisableNetworkPrediction.ps1'
 ...
 It 'keeps policy startup service task driver and device state unchanged through read-only paths'{
  ...
  (Snapshot-State)|Should -BeExactly $before
  try{& $script:providerPath -Action Apply -StatePath (Join-Path $TestDrive 'exp077-network-prediction-state.json') -WhatIf|Out-Null}catch{}
  (Snapshot-State)|Should -BeExactly $before
 }
}
