...
Describe 'EdgeDisableExtensionsPerformanceDetector integration' -Tag 'WindowsIntegration' {
 BeforeAll {
  ...
  function Snapshot-State {
   $policy=foreach($path in 'HKLM:\SOFTWARE\Policies\Microsoft\Edge','HKLM:\SOFTWARE\Policies\Microsoft\Edge\Recommended'){if(Test-Path $path){$key=Get-Item $path;[pscustomobject]@{Path=$path;Values=@($key.GetValueNames()|Sort-Object|ForEach-Object{[pscustomobject]@{Name=$_;Kind=$key.GetValueKind($_).ToString();Data=$key.GetValue($_,$null,[Microsoft.Win32.RegistryValueOptions]::DoNotExpandEnvironmentNames)}})}}else{[pscustomobject]@{Path=$path;Values=@()}}}
 ...
 It 'keeps policy startup service task driver and device state unchanged during read-only paths' {
  ...
  (Snapshot-State)|Should -BeExactly $before
  try{& $provider -Action Apply -ProfileFixturePath $TestDrive -StatePath (Join-Path $TestDrive 'exp130-detector-state.json') -WhatIf|Out-Null}catch{}
  (Snapshot-State)|Should -BeExactly $before
 }
}
