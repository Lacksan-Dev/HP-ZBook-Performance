Describe 'EdgeSleepingTabsTimeout15Minutes integration' -Tag 'WindowsIntegration' {
 BeforeAll{
  $script:providerPath=Join-Path $PSScriptRoot '..\providers\EdgeSleepingTabsTimeout15Minutes.ps1'
 ...
 It 'keeps policy startup service task driver and device state unchanged through read-only paths'{
  ...
  (Snapshot-State)|Should -BeExactly $before
  try{& $script:providerPath -Action Apply -StatePath (Join-Path $TestDrive 'exp073-sleeping-tabs-state.json') -WhatIf|Out-Null}catch{}
  (Snapshot-State)|Should -BeExactly $before
 }
}
