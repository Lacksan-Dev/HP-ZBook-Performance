...
Describe 'SetPointEventManagerDemandLaunch contract' {
 ...
 It 'requires SetPoint executable identity and rejects servicing paths' { foreach($token in 'SetPoint','SetPointII','Logitech'){ $text | Should -Match $token }; $text | Should -Match 'updat\|uninstall\|repair\|firmware\|dfu\|driver\|pairing\|setup\|install' }
