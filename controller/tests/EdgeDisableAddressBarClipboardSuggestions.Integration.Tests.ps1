Describe 'EdgeDisableAddressBarClipboardSuggestions integration' -Tag 'WindowsIntegration' {
 BeforeAll{
  $script:providerPath=Join-Path $PSScriptRoot '..\providers\EdgeDisableAddressBarClipboardSuggestions.ps1'
 ...
 It 'keeps policy startup service task driver and device state unchanged during read-only paths'{
  ...
  (Snapshot-State)|Should -BeExactly $before
  try{& $script:providerPath -Action Apply -ProfileFixturePath $TestDrive -StatePath (Join-Path $TestDrive 'exp151-state.json') -WhatIf|Out-Null}catch{}
  (Snapshot-State)|Should -BeExactly $before
 }
}
