Describe 'EXP-117 LogiTuneStartupFolder provider contract' {
    BeforeAll {
        $scriptPath = Join-Path $PSScriptRoot '..\providers\LogiTuneStartupFolder.ps1'
    ...
    It 'limits candidates to approved Startup folders and Logitech-signed Logi Tune executables' {
        $source | Should -Match 'SpecialFolder\]::Startup'
        $source | Should -Match 'SpecialFolder\]::CommonStartup'
        $source | Should -Match 'LogiTuneAgent'
        $source | Should -Match 'LogiTuneApp'
    ...
    It 'refuses rollback collision and scope drift' {
        ...
        $source | Should -Match 'Logi Tune target identity drift detected'
