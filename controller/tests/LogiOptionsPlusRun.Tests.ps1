Describe 'EXP-050 Logi Options+ controller provider' {
    BeforeAll {
        $controllerRoot = Split-Path $PSScriptRoot -Parent
        $providerPath = Join-Path $controllerRoot 'providers/LogiOptionsPlusRun.ps1'
    ...
    It 'limits mutation to the current-user Run key and recognized Logi Options+ identities' {
        $content=Get-Content -LiteralPath $providerPath -Raw
        $content | Should -Match [regex]::Escape('HKCU:\Software\Microsoft\Windows\CurrentVersion\Run')
        $content | Should -Match 'logioptionsplus_agent'
        $content | Should -Match 'logioptionsplus_appbroker'
    ...
    It 'registers a reversible profile with every protected scope' {
        ...
        $profile=@($manifest.profiles|Where-Object id -eq 'LogiOptionsPlusDemandLaunch')
        $profile.Count | Should -Be 1
        @($profile[0].providers) | Should -Contain 'logi-options-plus-run'
        $provider=@($manifest.providers|Where-Object id -eq 'logi-options-plus-run')[0]
