Describe 'EXP-049 classic Teams controller provider' {
    BeforeAll {
        $controllerRoot = Split-Path $PSScriptRoot -Parent
        $providerPath = Join-Path $controllerRoot 'providers/ClassicTeamsRun.ps1'
    ...
    It 'limits mutation to the current-user Run key and exact classic Teams identity' {
        $content=Get-Content -LiteralPath $providerPath -Raw
        $content | Should -Match [regex]::Escape('HKCU:\Software\Microsoft\Windows\CurrentVersion\Run')
        $content | Should -Match 'com\.squirrel\.Teams\.Teams'
        $content | Should -Match 'processStart'
        $content | Should -Match 'system-initiated'
        $content | Should -Match 'ms-teams'
    ...
    It 'registers a reversible profile with every protected scope' {
        ...
        $profile=@($manifest.profiles|Where-Object id -eq 'ClassicTeamsDemandLaunch')
        $profile.Count | Should -Be 1
        @($profile[0].providers) | Should -Contain 'classic-teams-run'
        $provider=@($manifest.providers|Where-Object id -eq 'classic-teams-run')[0]
