Describe 'EXP-071 Edge background mode policy provider' {
    BeforeAll {
        $controllerRoot=Split-Path $PSScriptRoot -Parent
        $providerPath=Join-Path $controllerRoot 'providers/EdgeBackgroundModePolicy.ps1'
    ...
    It 'limits mutation to the recommended Edge background mode policy' {
        $content=Get-Content $providerPath -Raw
        $content|Should -Match [regex]::Escape('HKLM:\SOFTWARE\Policies\Microsoft\Edge\Recommended')
        $content|Should -Match 'BackgroundModeEnabled'
    ...
    It 'registers one reversible profile with all protected scopes' {
        ...
        $profile=@($manifest.profiles|Where-Object id -eq 'EdgeBackgroundModeDisabled')
        $profile.Count|Should -Be 1
        @($profile[0].providers)|Should -Contain 'edge-background-mode-policy'
        $provider=@($manifest.providers|Where-Object id -eq 'edge-background-mode-policy')[0]
