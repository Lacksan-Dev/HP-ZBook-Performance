...
Describe 'EXP-063 HP App Helper HSA manual demand-start provider contract' {
 ...
 It 'targets exactly HPAppHelperCap and Manual startup only' {$text|Should -Match "ServiceName='HPAppHelperCap'";$text|Should -Match 'AppHelperCap\\.exe';$text|Should -Match 'Set-Service -Name \\$ServiceName -StartupType Manual';$text|Should -Not -Match 'Get-Service \\*|Set-Service .+\\*'}
 ...
 It 'requires exact display and executable identity' {$text|Should -Match 'HP App Helper HSA Service';$text|Should -Match 'display identity mismatch';$text|Should -Match 'executable identity mismatch'}
