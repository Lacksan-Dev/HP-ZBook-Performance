Describe 'EXP-122 LogiOptionsPlusStartupFolder zero-mutation integration' -Tag 'Integration' {
    BeforeAll {
        ...
        $provider = Join-Path $PSScriptRoot '..\providers\LogiOptionsPlusStartupFolder.ps1'
        $script:state = Join-Path $TestDrive 'exp122-state.json'
        $script:log = Join-Path $TestDrive 'exp122-events.jsonl'
        function Snapshot-System {
            [ordered]@{
                ...
                Run = @(
                    foreach($key in 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Run','HKLM:\Software\Microsoft\Windows\CurrentVersion\Run','HKCU:\Software\Microsoft\Windows\CurrentVersion\RunOnce','HKLM:\Software\Microsoft\Windows\CurrentVersion\RunOnce') {
