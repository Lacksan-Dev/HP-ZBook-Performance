Describe 'EXP-115 LogitechGHubStartupFolder zero-mutation integration' -Tag 'Integration' {
    BeforeAll {
        ...
        $provider = Join-Path $PSScriptRoot '..\providers\LogitechGHubStartupFolder.ps1'
        $script:state = Join-Path $TestDrive 'exp115-state.json'
        $script:log = Join-Path $TestDrive 'exp115-events.jsonl'
