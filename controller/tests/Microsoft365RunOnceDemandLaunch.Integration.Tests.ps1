...
Describe 'EXP-098 Microsoft365RunOnceDemandLaunch zero-mutation integration' -Tag 'WindowsIntegration' {
    BeforeAll {
        ...
        function Snapshot-State {
            ...
            [ordered]@{RunOnce=@($runOnce);Services=$services;Tasks=$tasks;Drivers=$drivers;Devices=$devices;Security=$security;ProtectedProcesses=$protected}|ConvertTo-Json -Compress -Depth 10
        }
    }
    It 'keeps registry service task driver device security and protected-process state unchanged during read-only paths' {
        if($env:LACKSAN_RUN_WINDOWS_INTEGRATION-ne'1' -or $env:OS-ne'Windows_NT'){Set-ItResult -Skipped -Because 'Opt-in HP Windows 11 integration only.';return}
        $before=Snapshot-State;$state=Join-Path $TestDrive 'exp098-state.json';$log=Join-Path $TestDrive 'exp098-events.jsonl'
