...
Describe 'EdgeDisableCopilotNewTabPage integration' -Tag 'WindowsIntegration' {
 BeforeAll {
 ...
 It 'keeps policy startup service task driver and device state unchanged during read-only paths' {
  ...
  (Snapshot-State)|Should -BeExactly $before
  try{& $provider -Action Apply -ProfileFixturePath $TestDrive -StatePath (Join-Path $TestDrive 'exp139-state.json') -WhatIf|Out-Null}catch{}
  (Snapshot-State)|Should -BeExactly $before
 }
}
