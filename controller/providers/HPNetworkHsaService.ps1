...
$protectedPattern='(?i)defender|firewall|bitlocker|credential|vbs|security|update|recovery|accessibility|omnissa|vmware|windows app|remote desktop|termservice|tailscale|driver|network|vpn|ndis|tcpip|dhcp|dns'
$hpRiskPattern='(?i)OMEN Gaming Hub|OMEN Command Center|HP.*Network|Network Switch'

function Write-StructuredLog([string]$Event,[string]$Result,[object]$Data){
    if($LogPath){
        $record=[ordered]@{timestampUtc=(Get-Date).ToUniversalTime().ToString('o');experiment='EXP-066';provider='hp-network-hsa-service';action=$Action;event=$Event;result=$Result;data=$Data}
...
function Get-NetworkBoundary{
    $adapters=@(Get-NetAdapter -ErrorAction SilentlyContinue | Select-Object Name,InterfaceDescription,Status,MacAddress,LinkSpeed,DriverInformation)
    $bindings=@(Get-NetAdapterBinding -ErrorAction SilentlyContinue | Select-Object Name,DisplayName,ComponentID,Enabled)
    $routes=@(Get-NetRoute -ErrorAction SilentlyContinue | Select-Object DestinationPrefix,NextHop,InterfaceAlias,RouteMetric,PolicyStore)
    $dns=@(Get-DnsClientServerAddress -ErrorAction SilentlyContinue | Select-Object InterfaceAlias,AddressFamily,ServerAddresses)
    $proxy=(Get-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings' -ErrorAction SilentlyContinue | Select-Object ProxyEnable,ProxyServer,AutoConfigURL)
    [pscustomobject]@{Adapters=$adapters;Bindings=$bindings;Routes=$routes;Dns=$dns;Proxy=$proxy}
}
function Get-ApplicationBoundary{
    $products=@()
    foreach($path in @('HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*','HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*')){
        $products+=@(Get-ItemProperty $path -ErrorAction SilentlyContinue | Where-Object {$_.DisplayName -match $hpRiskPattern} | Select-Object DisplayName,DisplayVersion,Publisher,InstallLocation,PSPath)
    }
    $packages=@(Get-AppxPackage -AllUsers -ErrorAction SilentlyContinue | Where-Object {$_.Name -match '(?i)OMEN|HP.*Network'} | Select-Object Name,PackageFullName,Version,Publisher)
    [pscustomobject]@{RiskProducts=$products;RiskPackages=$packages;ContinuousDependencyDetected=(@($products).Count -gt 0 -or @($packages).Count -gt 0)}
}
function Get-Support{
    ...
    $apps=Get-ApplicationBoundary
    $identity=($svc -and $svc.DisplayName -match '(?i)HP Network.*HSA|HP.*Network' -and $exe -match '(?i)NetworkCap\.exe$')
    ...
    [pscustomobject]@{
        Supported=($os.Caption -match 'Windows 11' -and $cs.Manufacturer -match '(?i)^HP$|Hewlett-Packard' -and $services.Count -eq 1 -and $identity -and $publisherOk -and $dependencySafe -and -not $management.Owned -and -not $apps.ContinuousDependencyDetected)
        Elevated=$admin;OS=$os.Caption;Build=$os.BuildNumber;Manufacturer=$cs.Manufacturer;Model=$cs.Model
        ServiceCount=$services.Count;Service=$svc;Executable=$exe;ExecutableVersion=$version;ExecutableHash=$hash;SignatureStatus=if($signature){$signature.Status}else{$null};Signer=if($signature -and $signature.SignerCertificate){$signature.SignerCertificate.Subject}else{$null}
        IdentityMatched=$identity;PublisherMatched=$publisherOk;Dependencies=$deps;DependentServices=$dependents;DependencySafe=$dependencySafe;Management=$management;Applications=$apps
    }
}
function Get-State{
    ...
    [pscustomobject]@{
        ...
        ExecutableHash=if($exe -and(Test-Path -LiteralPath $exe)){(Get-FileHash -LiteralPath $exe -Algorithm SHA256).Hash}else{$null}
        ...
        NetworkBoundary=Get-NetworkBoundary;ApplicationBoundary=Get-ApplicationBoundary
    }
}
function Save-State($State,$Support){
    if([string]::IsNullOrWhiteSpace($StatePath)){throw 'StatePath is required.'}
    $saved=[ordered]@{schemaVersion=1;experiment='EXP-066';provider='hp-network-hsa-service';machine=$env:COMPUTERNAME;capturedUtc=(Get-Date).ToUniversalTime().ToString('o');windowsBuild=$Support.Build;manufacturer=$Support.Manufacturer;model=$Support.Model;service=$State}
    $parent=Split-Path -Parent $StatePath;if($parent){New-Item -ItemType Directory -Path $parent -Force|Out-Null}
    $saved|ConvertTo-Json -Depth 14|Set-Content -LiteralPath $StatePath -Encoding UTF8
    $saved
}
function Read-State{
    ...
    if($saved.schemaVersion -ne 1 -or $saved.experiment -ne 'EXP-066' -or $saved.provider -ne 'hp-network-hsa-service' -or $saved.machine -ne $env:COMPUTERNAME -or $saved.service.Name -ne $serviceName){throw 'State identity validation failed.'}
    $saved
}
function Assert-Safe($Support,$State){
    if(-not $Support.Elevated){throw 'Elevation is required.'}
    if(-not $Support.Supported){throw 'Exact supported HP Windows 11 HP Network HSA service identity and safe application boundary are required.'}
    ...
    if($State.ApplicationBoundary.ContinuousDependencyDetected){throw 'OMEN or HP network application dependency requires physical review.'}
}
...
function Assert-CurrentIdentityMatchesSaved($Saved,$Current){
    foreach($field in @('Name','DisplayName','PathName','ExecutablePath','StartName','ExecutableVersion','ExecutableHash','SignatureStatus','Signer')){if([string]$Current.$field -ne [string]$Saved.service.$field){throw "Rollback refused because service identity drifted: $field"}}
    ...
    if(($Current.NetworkBoundary|ConvertTo-Json -Compress -Depth 12) -ne ($Saved.service.NetworkBoundary|ConvertTo-Json -Compress -Depth 12)){throw 'Rollback refused because protected network state drifted.'}
}
try{
    $support=Get-Support
    ...
    switch($Action){
        ...
        'DryRun'{
            ...
            else{Assert-Safe $support $state;$result=[pscustomobject]@{WouldChange=$true;Service=$serviceName;TargetStartMode='Manual';TargetDelayedAutoStart=0;PreserveRunningState=$true;NetworkMutationCount=0}}
        ...
        'Apply'{
            ...
            Write-StructuredLog 'apply' 'pass' @{changed=$true;networkMutationCount=0};[pscustomobject]@{Applied=$true;MutationCount=1}
        }
