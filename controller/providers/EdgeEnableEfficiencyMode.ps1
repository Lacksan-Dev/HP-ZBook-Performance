...
$Experiment='EXP-074'
$Provider='edge-enable-efficiency-mode'
...
$Treatment=1
$RelatedNames=@('EfficiencyMode','EfficiencyModeOnPowerEnabled','SleepingTabsEnabled','SleepingTabsTimeout','StartupBoostEnabled','BackgroundModeEnabled','HardwareAccelerationModeEnabled','NetworkPredictionOptions')
...
function Get-PowerObservation{
 ...
 [pscustomobject]@{BatteryPresent=[bool]$battery;EstimatedChargeRemaining=if($battery){$battery.EstimatedChargeRemaining}else{$null};BatteryStatus=if($battery){$battery.BatteryStatus}else{$null}}
}
...
function Get-Support{
 ...
 $supported=($os.Caption-match'Windows 11'-and$computer.Manufacturer-match'(?i)^HP$|Hewlett-Packard'-and(Test-Elevated)-and$edge-and$edge.ValidPublisher-and$edge.Major-ge106-and!$management.Managed-and!$policies.MandatoryCandidate.ValueExists-and!$policies.RecommendedCandidate.ValueExists-and$startup.EdgeEntryCount-eq0)
 [pscustomobject]@{Supported=$supported;OS=$os.Caption;Build=$os.BuildNumber;Manufacturer=$computer.Manufacturer;Model=$computer.Model;Elevated=(Test-Elevated);Edge=$edge;Management=$management;Policies=$policies;Startup=$startup;Power=Get-PowerObservation;EvidenceStatus='needs-evidence'}
}
function Assert-Supported($Support){if(!$Support.Supported){throw 'Unsupported, managed, ambiguous, pre-106 Edge, existing EfficiencyModeEnabled policy, unsigned Edge, or Edge Startup-folder state.'}}
...
function Assert-DriftFree($State){
 $edge=Get-Edge;if(!$edge-or$edge.Major-lt106-or$edge.Sha256-ne$State.edge.Sha256-or$edge.Version-ne$State.edge.Version-or$edge.Thumbprint-ne$State.edge.Thumbprint){throw 'Edge identity drift detected.'};if((Get-Management).Managed){throw 'Management ownership drift detected.'}
 $policies=Get-Policies;if(!(Test-Same $policies.Related $State.related)-or!(Test-Same $policies.MandatoryCandidate $State.mandatoryOriginal)){throw 'Related Edge policy drift detected.'};if((Get-StartupFolders).EdgeEntryCount-ne0){throw 'Edge Startup-folder drift detected.'};$protected=Get-Protected;if(!(Test-Same $protected.Configuration $State.protectedConfiguration)){throw 'Protected service configuration drift detected.'};[pscustomobject]@{ProtectedRuntime=$protected.Runtime;Power=Get-PowerObservation}
}
...
try{
 ...
 switch($Action){
  'Check'{[pscustomobject]@{Support=$support;Profile='EdgeEnableEfficiencyMode'}}
  ...
  'Apply'{$state=if($StatePath-and(Test-Path -LiteralPath $StatePath)){Read-State}else{Save-State $support};$drift=Assert-DriftFree $state;if(Test-Applied){Write-Log 'apply' 'idempotent' @{mutationCount=0;protectedRuntime=$drift.ProtectedRuntime;power=$drift.Power};return [pscustomobject]@{Applied=$true;MutationCount=0}};Assert-Supported $support;if($WhatIfPreference){Write-Log 'apply' 'whatif' @{mutationCount=0};return [pscustomobject]@{Applied=$false;WhatIf=$true;MutationCount=0}};Assert-EdgeClosed;if($PSCmdlet.ShouldProcess("$RecommendedPath::$ValueName",'Enable Edge efficiency mode')){if(!(Test-Path -LiteralPath $RecommendedPath)){New-Item -Path $RecommendedPath -Force|Out-Null};New-ItemProperty -LiteralPath $RecommendedPath -Name $ValueName -PropertyType DWord -Value $Treatment -Force|Out-Null};if(!(Test-Applied)){throw 'Apply verification failed.'};Write-Log 'apply' 'pass' @{mutationCount=1;protectedRuntime=$drift.ProtectedRuntime;power=$drift.Power};[pscustomobject]@{Applied=$true;MutationCount=1}}
  'Verify'{$state=Read-State;$drift=Assert-DriftFree $state;if(!(Test-Applied)){throw 'Policy verification failed.'};$result=[pscustomobject]@{Verified=$true;ProtectedRuntime=$drift.ProtectedRuntime;Power=$drift.Power;EvidenceStatus='needs-evidence'};Write-Log 'verify' 'pass' $result;$result}
  'VerifyReboot'{$state=Read-State;$drift=Assert-DriftFree $state;$boot=(Get-CimInstance Win32_OperatingSystem).LastBootUpTime.ToUniversalTime();if($boot-le[datetime]$state.capturedBootTime){throw 'A later boot is required.'};if(!(Test-Applied)){throw 'Reboot persistence failed.'};$result=[pscustomobject]@{VerifiedReboot=$true;BootTime=$boot.ToString('o');ProtectedRuntime=$drift.ProtectedRuntime;Power=$drift.Power;EvidenceStatus='needs-evidence'};Write-Log 'verify-reboot' 'pass' $result;$result}
