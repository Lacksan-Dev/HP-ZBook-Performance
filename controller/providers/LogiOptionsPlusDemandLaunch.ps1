...
$experiment='EXP-050'
$provider='logi-options-plus-demand-launch'
...
function Resolve-Command([string]$Command){
 ...
 if($exe -notmatch '(?i)\\LogiOptionsPlus\\.*\\?(logioptionsplus_agent|logioptionsplus|logioptionsplus_appbroker)\.exe$'){return $null}
 if($exe -match '(?i)updat|uninstall|repair|firmware'){return $null}
 if($args -and $args -notmatch '(?i)background|minimi[sz]ed|startup|tray|silent|^\s*$'){return $null}
...
function Get-Candidates{
 ...
 foreach($n in $k.GetValueNames()){
  if($n -notmatch '(?i)logi.*options|options\+'){continue};$d=[string]$k.GetValue($n,$null,[Microsoft.Win32.RegistryValueOptions]::DoNotExpandEnvironmentNames);$r=Resolve-Command $d;if(!$r){continue};$id=Get-FileIdentity $r.Executable;if(!$id -or !$id.ValidLogitechPublisher){continue}
...
function Assert-Eligible($Support,[object[]]$Candidates){if(!$Support.Supported){throw 'HP Windows 11 is required.'};if($Support.Managed){throw 'Enterprise-management ownership detected.'};if($Candidates.Count -ne 1){throw "Exactly one eligible Logi Options+ Run registration is required; found $($Candidates.Count)."}}
...
function Assert-Executable($State){$e=Get-FileIdentity ([string]$State.entry.Executable.Path);if(!$e -or !$e.ValidLogitechPublisher -or $e.Sha256 -ne [string]$State.entry.Executable.Sha256 -or $e.Thumbprint -ne [string]$State.entry.Executable.Thumbprint){throw 'Logi Options+ executable identity drift detected.'}}
...
try{
 ...
 switch($Action){
  'Check'{$c=Get-Candidates;Write-Log 'candidate-inventory' 'pass' @{count=$c.Count;names=@($c.Name)};[pscustomobject]@{Support=$support;Candidates=$c;Profile='LogiOptionsPlusDemandLaunch'}}
  'Capture'{$s=Save-State $support (Get-Candidates);Write-Log 'capture' 'pass' @{path=$s.entry.Path;name=$s.entry.Name};$s}
  'DryRun'{$c=Get-Candidates;Assert-Eligible $support $c;$r=[pscustomobject]@{Profile='LogiOptionsPlusDemandLaunch';WouldChange=$true;MutationCount=1;Path=$c[0].Path;Name=$c[0].Name;PreserveApplication=$true;PreserveDrivers=$true;RebootPersistenceCheckRequired=$true;Rollback='Restore exact captured value name, kind, and unexpanded data.'};Write-Log 'dry-run' 'pass' $r;$r}
