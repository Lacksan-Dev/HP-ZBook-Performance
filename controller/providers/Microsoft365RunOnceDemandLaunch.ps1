...
$experiment='EXP-098'
$provider='microsoft-365-runonce-demand-launch'
$profile='Microsoft365RunOnceDemandLaunch'
...
function Assert-Eligible($Support,[object[]]$Candidates){
    if(!$Support.Supported){throw 'HP Windows 11 is required.'};if($Support.Managed){throw 'Enterprise-management ownership detected.'};if($Candidates.Count-ne1){throw "Exactly one eligible Microsoft 365 RunOnce registration is required; found $($Candidates.Count)."};if($Candidates[0].Hive-eq'HKLM'-and!$Support.Elevated){throw 'Elevation is required for HKLM RunOnce.'}
}
function Save-State($Support,[object[]]$Candidates){
    Assert-Eligible $Support $Candidates;if([string]::IsNullOrWhiteSpace($StatePath)){throw 'StatePath is required.'};if(Test-Path -LiteralPath $StatePath){throw 'State overwrite refused.'};$p=Get-ProtectedSnapshot
...
try{
    ...
    switch($Action){
        ...
        'Capture'{$s=Save-State $support (Get-Candidates);Write-Log 'capture' 'pass' @{path=$s.entry.Path;name=$s.entry.Name;kind=$s.entry.Kind;product=$s.entry.Product.Name;version=$s.entry.Product.Version;exeSha256=$s.entry.Executable.Sha256};$s}
        'DryRun'{$c=Get-Candidates;Assert-Eligible $support $c;$r=[pscustomobject]@{Profile=$profile;WouldChange=$true;MutationCount=1;Path=$c[0].Path;Name=$c[0].Name;Product=$c[0].Product.Name;FromPresent=$true;ToPresent=$false;PreserveMicrosoft365=$true;PreserveClickToRun=$true;PreserveActivation=$true;PreserveUpdates=$true;PreserveDocuments=$true;PreserveAddins=$true;OneShotConsumptionEvidenceRequired=$true;RebootPersistenceCheckRequired=$true;Rollback='Restore the exact captured registry value name, type, and unexpanded data after drift checks.'};Write-Log 'dry-run' 'pass' $r;$r}
        'Apply'{$s=if($StatePath-and(Test-Path -LiteralPath $StatePath)){Read-State}else{Save-State $support (Get-Candidates)};if(Test-Removed $s){Assert-Executable $s;Write-Log 'apply' 'idempotent' @{mutationCount=0};return [pscustomobject]@{Applied=$true;MutationCount=0}};Assert-Eligible $support (Get-Candidates);Assert-Executable $s;if((Get-ProtectedSnapshot).Hash-ne[string]$s.protectedScopeHash){throw 'Protected-scope drift detected.'};$k=Get-Item -LiteralPath ([string]$s.entry.Path);$data=[string]$k.GetValue([string]$s.entry.Name,$null,[Microsoft.Win32.RegistryValueOptions]::DoNotExpandEnvironmentNames);if($data-ne[string]$s.entry.Data-or$k.GetValueKind([string]$s.entry.Name).ToString()-ne[string]$s.entry.Kind){throw 'RunOnce registration drift detected.'};if($WhatIfPreference){Write-Log 'apply' 'whatif' @{mutationCount=0};return [pscustomobject]@{Applied=$false;WhatIf=$true;MutationCount=0}};if($PSCmdlet.ShouldProcess("$($s.entry.Path)::$($s.entry.Name)",'Remove exact Microsoft 365 RunOnce registration')){Remove-ItemProperty -LiteralPath ([string]$s.entry.Path) -Name ([string]$s.entry.Name)};if(!(Test-Removed $s)){throw 'Apply verification failed.'};Write-Log 'apply' 'pass' @{mutationCount=1;oneShotAttribution='needs-evidence'};[pscustomobject]@{Applied=$true;MutationCount=1;NeedsEvidence='Controlled sign-in attribution and Microsoft 365/protected-application readiness'}}
        'Verify'{$s=Read-State;if(!(Test-Removed $s)){throw 'Immediate verification failed.'};Assert-Executable $s;Write-Log 'verify' 'pass' @{removed=$true;oneShotAttribution='needs-evidence'};$true}
        'VerifyReboot'{$s=Read-State;$boot=(Get-CimInstance Win32_OperatingSystem).LastBootUpTime.ToUniversalTime();if($boot-le[datetime]$s.capturedBootTime){throw 'A later boot is required.'};if(!(Test-Removed $s)){throw 'Reboot persistence failed.'};Assert-Executable $s;Write-Log 'verify-reboot' 'pass' @{bootTime=$boot.ToString('o');oneShotAttribution='needs-evidence'};$true}
        'Rollback'{$s=Read-State;if(!(Test-Removed $s)){if(Test-Restored $s){Write-Log 'rollback' 'idempotent' @{mutationCount=0;restoredExactOriginal=$true};return [pscustomobject]@{RolledBack=$true;MutationCount=0}};throw 'Rollback overwrite refused.'};$m=Get-ManagementState;if($m.Managed){throw 'Enterprise-management ownership appeared.'};Assert-Executable $s;if((Get-ProtectedSnapshot).Hash-ne[string]$s.protectedScopeHash){throw 'Protected-scope drift detected.'};if($WhatIfPreference){Write-Log 'rollback' 'whatif' @{mutationCount=0};return [pscustomobject]@{RolledBack=$false;WhatIf=$true;MutationCount=0}};if($PSCmdlet.ShouldProcess("$($s.entry.Path)::$($s.entry.Name)",'Restore exact Microsoft 365 RunOnce registration')){if(!(Test-Path -LiteralPath ([string]$s.entry.Path))){New-Item -Path ([string]$s.entry.Path) -Force|Out-Null};(Get-Item -LiteralPath ([string]$s.entry.Path)).SetValue([string]$s.entry.Name,[string]$s.entry.Data,[Microsoft.Win32.RegistryValueKind]::$($s.entry.Kind))};if(!(Test-Restored $s)){throw 'Exact rollback verification failed.'};Write-Log 'rollback' 'pass' @{mutationCount=1;restoredExactOriginal=$true;postRollbackSignIn='needs-evidence'};[pscustomobject]@{RolledBack=$true;MutationCount=1;NeedsEvidence='Controlled post-rollback sign-in must distinguish normal RunOnce consumption from rollback correctness'}}
    }
}catch{Write-Log 'failure' 'fail' @{stage=$Action;message=$_.Exception.Message;type=$_.Exception.GetType().FullName;stack=$_.ScriptStackTrace};throw}
