...
$Experiment='EXP-073'
$Provider='edge-sleeping-tabs-timeout-15-minutes'
...
$Treatment=900
$RelatedNames=@('SleepingTabsEnabled','StartupBoostEnabled','BackgroundModeEnabled','HardwareAccelerationModeEnabled','NetworkPredictionOptions','RestoreOnStartup','ClearBrowsingDataOnExit','ClearCachedImagesAndFilesOnExit')
...
function Assert-Supported($Support){if(!$Support.Supported){throw 'Unsupported, managed, ambiguous, pre-88 Edge, existing SleepingTabsTimeout policy, unsigned Edge, or Edge Startup-folder state.'}}
...
try{
 ...
 switch($Action){
  'Check'{[pscustomobject]@{Support=$support;Profile='EdgeSleepingTabsTimeout15Minutes'}}
