...
function Get-PackageInventory([string]$Executable){
    $parent=Split-Path -Parent $Executable;$items=@();foreach($root in 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'){
        Get-ChildItem $root -ErrorAction SilentlyContinue|ForEach-Object{$p=Get-ItemProperty $_.PSPath -ErrorAction SilentlyContinue;if($p.Publisher-match'(?i)HP|Hewlett-Packard' -and (($p.DisplayName-match'(?i)System Info|System Information|HSA') -or ($p.InstallLocation -and $parent.StartsWith([string]$p.InstallLocation,[StringComparison]::OrdinalIgnoreCase)))){$items+=[pscustomobject]@{Key=$_.PSChildName;DisplayName=$p.DisplayName;DisplayVersion=$p.DisplayVersion;Publisher=$p.Publisher;InstallLocation=$p.InstallLocation}}}
    };@($items)
}
