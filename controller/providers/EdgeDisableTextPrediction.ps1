...
$Experiment='EXP-149'
$Provider='edge-disable-text-prediction'
$PolicyPath='HKLM:\SOFTWARE\Policies\Microsoft\Edge'
$ValueName='TextPredictionEnabled'
$RelatedNames=@('SearchSuggestEnabled','AddressBarWorkSearchResultsEnabled','CopilotAddressBarSuggestionsEnabled','AddressBarClipboardSuggestEnabled','EdgeShoppingAssistantEnabled','NewTabPageContentEnabled','NewTabPagePrerenderEnabled','NewTabPageLocation','StartupBoostEnabled','BackgroundModeEnabled','SleepingTabsEnabled','HardwareAccelerationModeEnabled','NetworkPredictionOptions','RestoreOnStartup')
...
function Get-Edge{
 ...
 if($items.Count-ne1){return $null}
 $items[0]
}
...
function Get-Fixture{
 ...
 $marker=Join-Path $ProfileFixturePath '.lacksan-edge-text-prediction.json'
 if(!(Test-Path -LiteralPath $marker)){return $null}
 $data=Get-Content -LiteralPath $marker -Raw|ConvertFrom-Json
 if($data.experiment-ne'EXP-149'-or$data.controlledProfile-ne$true-or$data.microsoftAccountSignedIn-ne$false-or$data.textPredictionEligible-ne$true-or[string]::IsNullOrWhiteSpace([string]$data.fixtureSha256)){return $null}
 [pscustomobject]@{Path=(Resolve-Path -LiteralPath $ProfileFixturePath).Path;Marker=$marker;MarkerSha256=(Get-FileHash -LiteralPath $marker -Algorithm SHA256).Hash;Identity=[string]$data.profileId;MicrosoftAccountSignedIn=[bool]$data.microsoftAccountSignedIn;TextPredictionEligible=[bool]$data.textPredictionEligible;FixtureSha256=[string]$data.fixtureSha256;ControlledProfile=$true}
}
...
function Get-Policies{
 $related=[ordered]@{}
 foreach($name in $RelatedNames){$related[$name]=Get-Reg $PolicyPath $name;$related["Recommended:$name"]=Get-Reg "$PolicyPath\Recommended" $name}
 [pscustomobject]@{Candidate=Get-Reg $PolicyPath $ValueName;RecommendedCandidate=Get-Reg "$PolicyPath\Recommended" $ValueName;Related=[pscustomobject]$related}
}
...
function Get-Support{
 $os=Get-CimInstance Win32_OperatingSystem;$computer=Get-CimInstance Win32_ComputerSystem;$edge=Get-Edge;$management=Get-Management;$fixture=Get-Fixture;$policies=Get-Policies;$startup=Get-StartupFolders
 $supported=($os.Caption-match'Windows 11'-and$computer.Manufacturer-match'(?i)^HP$|Hewlett-Packard'-and(Test-Elevated)-and$edge-and$edge.ValidPublisher-and$edge.Major-ge104-and!$management.Managed-and$fixture-and!$policies.Candidate.ValueExists-and!$policies.RecommendedCandidate.ValueExists-and$startup.EdgeEntryCount-eq0)
 [pscustomobject]@{Supported=$supported;OS=$os.Caption;Build=$os.BuildNumber;Manufacturer=$computer.Manufacturer;Model=$computer.Model;Elevated=(Test-Elevated);Edge=$edge;Management=$management;Fixture=$fixture;Policies=$policies;Startup=$startup;EvidenceStatus='needs-evidence'}
}
function Assert-Supported($Support){if(!$Support.Supported){throw 'Unsupported, managed, ambiguous, pre-104 Edge, existing-policy, ineligible-profile, uncontrolled-text-fixture, unsigned Edge, or Startup-folder state.'}}
...
function Assert-DriftFree($State){
 $edge=Get-Edge;if(!$edge-or$edge.Major-lt104-or$edge.Sha256-ne$State.edge.Sha256-or$edge.Version-ne$State.edge.Version-or$edge.Thumbprint-ne$State.edge.Thumbprint){throw 'Edge identity drift detected.'}
 if((Get-Management).Managed){throw 'Management ownership drift detected.'}
 $fixture=Get-Fixture;if(!$fixture-or$fixture.MarkerSha256-ne$State.fixture.MarkerSha256-or$fixture.Identity-ne$State.fixture.Identity-or$fixture.MicrosoftAccountSignedIn-or!$fixture.TextPredictionEligible-or$fixture.FixtureSha256-ne$State.fixture.FixtureSha256){throw 'Controlled profile or text fixture drift detected.'}
 $policies=Get-Policies;if(!(Test-Same $policies.Related $State.related)-or!(Test-Same $policies.RecommendedCandidate $State.recommendedOriginal)){throw 'Related Edge policy drift detected.'}
 if((Get-StartupFolders).EdgeEntryCount-ne0){throw 'Edge Startup-folder drift detected.'}
 $protected=Get-Protected;if(!(Test-Same $protected.Configuration $State.protectedConfiguration)){throw 'Protected service configuration drift detected.'}
 [pscustomobject]@{ProtectedRuntime=$protected.Runtime}
}
function Test-Applied{$candidate=Get-Reg $PolicyPath $ValueName;$candidate.ValueExists-and$candidate.Kind-eq'DWord'-and[int]$candidate.Data-eq0}
try{
 $support=Get-Support;Write-Log 'support-detection' $(if($support.Supported){'pass'}else{'unsupported'}) $support
 switch($Action){
  'Check'{[pscustomobject]@{Support=$support;Profile='EdgeDisableTextPrediction'}}
  'Capture'{$state=Save-State $support;Write-Log 'capture' 'pass' $state;$state}
  'DryRun'{Assert-Supported $support;$result=[pscustomobject]@{WouldChange=$true;MutationCount=1;Path=$PolicyPath;Name=$ValueName;Type='DWord';Value=0;BrowserRestartRequired=$true;RebootPersistenceCheckRequired=$true;Rollback='Restore exact captured registry value/type or remove only the experiment-created value and empty experiment-created key.';EvidenceStatus='needs-evidence'};Write-Log 'dry-run' 'pass' $result;$result}
  'Apply'{$state=if($StatePath-and(Test-Path -LiteralPath $StatePath)){Read-State}else{Save-State $support};$drift=Assert-DriftFree $state;if(Test-Applied){Write-Log 'apply' 'idempotent' @{mutationCount=0;protectedRuntime=$drift.ProtectedRuntime};return [pscustomobject]@{Applied=$true;MutationCount=0}};Assert-Supported $support;if($WhatIfPreference){Write-Log 'apply' 'whatif' @{mutationCount=0};return [pscustomobject]@{Applied=$false;WhatIf=$true;MutationCount=0}};Assert-EdgeClosed;if($PSCmdlet.ShouldProcess("$PolicyPath::$ValueName",'Disable Edge text prediction for controlled-navigation comparison')){if(!(Test-Path -LiteralPath $PolicyPath)){New-Item -Path $PolicyPath -Force|Out-Null};New-ItemProperty -LiteralPath $PolicyPath -Name $ValueName -PropertyType DWord -Value 0 -Force|Out-Null};if(!(Test-Applied)){throw 'Apply verification failed.'};Write-Log 'apply' 'pass' @{mutationCount=1;protectedRuntime=$drift.ProtectedRuntime};[pscustomobject]@{Applied=$true;MutationCount=1}}
  ...
  'Rollback'{$state=Read-State;$drift=Assert-DriftFree $state;Assert-EdgeClosed;$now=Get-Reg $PolicyPath $ValueName;if(!$now.ValueExists-and!$state.original.ValueExists){Write-Log 'rollback' 'idempotent' @{mutationCount=0;protectedRuntime=$drift.ProtectedRuntime};return [pscustomobject]@{RolledBack=$true;MutationCount=0}};if($state.original.ValueExists){if($now.ValueExists-and!(Test-Same $now ([pscustomobject]@{KeyExists=$true;ValueExists=$true;Kind='DWord';Data=0}))){throw 'Policy drift detected; rollback overwrite refused.'};$kind=[Enum]::Parse([Microsoft.Win32.RegistryValueKind],[string]$state.original.Kind,$true);$key=Get-Item -LiteralPath $PolicyPath;$key.SetValue($ValueName,$state.original.Data,$kind)}else{if($now.ValueExists-and!(Test-Applied)){throw 'Policy drift detected; rollback overwrite refused.'};if($now.ValueExists){Remove-ItemProperty -LiteralPath $PolicyPath -Name $ValueName};if(!$state.original.KeyExists-and(Test-Path -LiteralPath $PolicyPath)-and@((Get-Item -LiteralPath $PolicyPath).GetValueNames()).Count-eq0-and@((Get-ChildItem -LiteralPath $PolicyPath -ErrorAction SilentlyContinue)).Count-eq0){Remove-Item -LiteralPath $PolicyPath}};$after=Get-Reg $PolicyPath $ValueName;if(!(Test-Same $after $state.original)){throw 'Exact rollback verification failed.'};$post=Get-Protected;if(!(Test-Same $post.Configuration $state.protectedConfiguration)){throw 'Protected configuration changed during rollback.'};Write-Log 'rollback' 'pass' @{restoredExactOriginal=$true;protectedRuntime=$post.Runtime};[pscustomobject]@{RolledBack=$true;RestoredExactOriginal=$true}}
 }
