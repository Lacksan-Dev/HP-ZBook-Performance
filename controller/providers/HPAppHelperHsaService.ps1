...
function Write-StructuredLog([string]$Event,[string]$Result,[object]$Data){
    if($LogPath){
        $record=[ordered]@{timestampUtc=(Get-Date).ToUniversalTime().ToString('o');experiment='EXP-063';provider='hp-app-helper-hsa-service';action=$Action;event=$Event;result=$Result;data=$Data}
...
function Get-Support{
    ...
    $identity=($svc -and $svc.DisplayName -match '(?i)HP App Helper.*HSA|HP.*App Helper' -and $exe -match '(?i)HP.*AppHelper|AppHelper.*HP|HPAppHelper')
...
function Save-State($State,$Support){
    ...
    $saved=[ordered]@{schemaVersion=1;experiment='EXP-063';provider='hp-app-helper-hsa-service';machine=$env:COMPUTERNAME;capturedUtc=(Get-Date).ToUniversalTime().ToString('o');windowsBuild=$Support.Build;manufacturer=$Support.Manufacturer;model=$Support.Model;service=$State}
...
function Read-State{
    ...
    if($saved.schemaVersion -ne 1 -or $saved.experiment -ne 'EXP-063' -or $saved.provider -ne 'hp-app-helper-hsa-service' -or $saved.machine -ne $env:COMPUTERNAME -or $saved.service.Name -ne $serviceName){throw 'State identity validation failed.'}
    $saved
}
function Assert-Safe($Support,$State){
    if(-not $Support.Elevated){throw 'Elevation is required.'}
    if(-not $Support.Supported){throw 'Exact supported HP Windows 11 HP App Helper HSA service identity is required.'}
