...
function Get-HPManagementSignals{
    $serviceNames=@(Get-CimInstance Win32_Service -ErrorAction SilentlyContinue|Where-Object{$_.DisplayName-match'(?i)HP (Insights|TechPulse|Workforce Experience|Managed|Wolf.*Manage)' -or $_.Name-match'(?i)TechPulse|HPInsights|Workforce'}|Select-Object -ExpandProperty Name)
    $products=@();foreach($root in 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'){Get-ChildItem $root -ErrorAction SilentlyContinue|ForEach-Object{$p=Get-ItemProperty $_.PSPath -ErrorAction SilentlyContinue;if($p.DisplayName-match'(?i)HP (Insights|TechPulse|Workforce Experience|Managed Services)'){$products+=[string]$p.DisplayName}}}
    [pscustomobject]@{ServiceNames=@($serviceNames);Products=@($products);Detected=($serviceNames.Count-gt0-or$products.Count-gt0)}
}
function Get-ManagementState{
    $computer=Get-CimInstance Win32_ComputerSystem;$hp=Get-HPManagementSignals
    $signals=[ordered]@{
        ...
        HPManagedServices=$hp
    }
    [pscustomobject]@{Managed=($signals.DomainJoined-or$signals.MdmEnrollments-gt0-or$signals.PolicyManager-or$signals.ConfigMgr-or$signals.IntuneManagementExtension-or$hp.Detected);Signals=$signals}
}
function Get-SupportState{
    $os=Get-CimInstance Win32_OperatingSystem;$computer=Get-CimInstance Win32_ComputerSystem;$bios=Get-CimInstance Win32_BIOS;$m=Get-ManagementState;$elevated=Test-Elevated
    [pscustomobject]@{Supported=($os.Caption-match'Windows 11'-and$computer.Manufacturer-match'(?i)^HP$|Hewlett-Packard'-and$elevated);Elevated=$elevated;OS=$os.Caption;Build=$os.BuildNumber;Manufacturer=$computer.Manufacturer;Model=$computer.Model;BIOS=$bios.SMBIOSBIOSVersion;Managed=$m.Managed;ManagementSignals=$m.Signals}
}
...
function Get-PackageInventory([string]$Executable){
    $parent=Split-Path -Parent $Executable;$roots='HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
...
function Get-OneAgentTasks([string]$ServiceName){
    $items=foreach($task in Get-ScheduledTask -ErrorAction SilentlyContinue){
        if($task.TaskName-match'(?i)HpOneAgent.*(Repair|Update)' -or $task.TaskPath-match'(?i)HP.*OneAgent'){
            ...
            [ordered]@{TaskName=$task.TaskName;TaskPath=$task.TaskPath;State=$task.State.ToString();DefinitionHash=Get-TextHash $xml;Enabled=($task.State.ToString()-ne'Disabled');MentionsService=($xml-match[regex]::Escape($ServiceName));RepairOrUpdate=($task.TaskName-match'(?i)repair|update' -or $xml-match'(?i)repair|update')}
        }
    }
    @($items)
}
function Get-Identity{
    $matches=@(Get-CimInstance Win32_Service|Where-Object{$_.DisplayName-match'(?i)^HP One Agent Service$'});if($matches.Count-ne1){return $null}
    ...
    $file=Get-Item -LiteralPath $exe;$signature=Get-AuthenticodeSignature -LiteralPath $exe;$publisher=if($signature.SignerCertificate){$signature.SignerCertificate.Subject}else{$null};try{$version=[version]$file.VersionInfo.FileVersion}catch{return $null}
    ...
    $dependencies=@($service.Dependencies);$dependents=@((Get-Service $service.Name).DependentServices|ForEach-Object{$_.Name});$failure=(& sc.exe qfailure $service.Name 2>&1|Out-String).Trim();$triggers=(& sc.exe qtriggerinfo $service.Name 2>&1|Out-String).Trim();$tasks=Get-OneAgentTasks $service.Name;$packages=Get-PackageInventory $exe
    [pscustomobject]@{Name=$service.Name;DisplayName=$service.DisplayName;State=$service.State;StartMode=$service.StartMode;DelayedAutoStart=[int]($null-ne$delayed-and[int]$delayed-ne0);PathName=$service.PathName;ServiceAccount=$service.StartName;Dependencies=@($dependencies);Dependents=@($dependents);RecoveryActions=$failure;Triggers=$triggers;Tasks=@($tasks);Packages=@($packages);Executable=[ordered]@{Path=$file.FullName;Sha256=(Get-FileHash -LiteralPath $exe -Algorithm SHA256).Hash;FileVersion=$file.VersionInfo.FileVersion;ParsedVersion=$version.ToString();ProductName=$file.VersionInfo.ProductName;CompanyName=$file.VersionInfo.CompanyName;SignatureStatus=$signature.Status.ToString();Publisher=$publisher;Thumbprint=if($signature.SignerCertificate){$signature.SignerCertificate.Thumbprint}else{$null};ValidPublisher=($signature.Status-eq'Valid'-and$publisher-match'(?i)HP Inc|Hewlett-Packard');SecurityFloorMet=($version-ge$minimumSafeVersion)}}
}
function Assert-Eligible($Support,$Identity){
    if(!$Support.Supported){throw 'Elevated HP Windows 11 is required.'};if($Support.Managed){throw 'Enterprise or HP management ownership detected.'};if($null-eq$Identity){throw 'Exactly one verified HP One Agent Service identity is required.'};if(!$Identity.Executable.ValidPublisher){throw 'Valid HP publisher signature is required.'};if(!$Identity.Executable.SecurityFloorMet){throw "HP One Agent must be version $minimumSafeVersion or later before experimentation."};if($Identity.StartMode-notin @('Auto','Automatic')){throw 'Baseline startup mode must be Automatic.'};if($Identity.DelayedAutoStart-ne0){throw 'Service is already delayed.'};if($Identity.Dependencies-match$protected -or $Identity.Dependents-match$protected){throw 'Protected dependency detected.'};if($Identity.RecoveryActions-match'(?im)^\s*REBOOT_MESSAGE\s*:\s*\S+' -or $Identity.RecoveryActions-match'(?im)^\s*COMMAND_LINE\s*:\s*\S+'){throw 'Recovery-sensitive configuration detected.'};if(@($Identity.Tasks|Where-Object{$_.RepairOrUpdate-and$_.Enabled}).Count-gt0){throw 'Enabled HP One Agent repair/update task may overwrite treatment; application refused.'};if($Identity.Packages.Count-lt1){throw 'HP One Agent package/product ownership could not be established.'}
}
function Save-State($Support,$Identity){
    Assert-Eligible $Support $Identity;if([string]::IsNullOrWhiteSpace($StatePath)){throw 'StatePath is required.'};if(Test-Path -LiteralPath $StatePath){throw 'State overwrite refused.'}
...
function Assert-Identity($State){
    $current=Get-Identity;if($null-eq$current){throw 'HP One Agent identity drift detected.'};if($current.Name-ne[string]$State.service.Name-or$current.PathName-ne[string]$State.service.PathName-or$current.Executable.Sha256-ne[string]$State.service.Executable.Sha256-or$current.Executable.Thumbprint-ne[string]$State.service.Executable.Thumbprint-or$current.Executable.ParsedVersion-ne[string]$State.service.Executable.ParsedVersion){throw 'Service, executable, or version drift detected.'}
    $depJson=[ordered]@{Dependencies=@($current.Dependencies);Dependents=@($current.Dependents)}|ConvertTo-Json -Compress -Depth 8;if((Get-TextHash $depJson)-ne[string]$State.dependencyHash){throw 'Dependency drift detected.'};$taskJson=@($current.Tasks)|ConvertTo-Json -Compress -Depth 8;if((Get-TextHash $taskJson)-ne[string]$State.taskHash){throw 'HP One Agent repair/update task drift detected.'};$current
}
function Set-DelayedStart([string]$Name,[bool]$Delayed){$start=if($Delayed){'delayed-auto'}else{'auto'};$output=& sc.exe config $Name start= $start 2>&1;if($LASTEXITCODE-ne0){throw "sc.exe configuration failed: $($output|Out-String)"}}
