...
function Get-Fixture {
    ...
    $m=Join-Path $ProfileFixturePath '.lacksan-pristine-edge-profile.json';if(!(Test-Path -LiteralPath $m)){return $null};$x=Get-Content -LiteralPath $m -Raw|ConvertFrom-Json
    if($x.experiment -ne 'EXP-138' -or $x.controlledPristineProfile -ne $true -or $x.firstRunComplete -ne $false){return $null}
    [pscustomobject]@{Path=(Resolve-Path -LiteralPath $ProfileFixturePath).Path;Marker=$m;MarkerSha256=(Get-FileHash -LiteralPath $m -Algorithm SHA256).Hash;Identity=[string]$x.profileId;FirstRunComplete=[bool]$x.firstRunComplete;ProductionProfile=$false}
}
...
function Assert-EdgeClosed {if(Get-Process msedge -ErrorAction SilentlyContinue){throw 'Close all Edge processes before policy application or rollback so the restart boundary is explicit.'}}
...
function Assert-DriftFree($s){
    ...
    $f=Get-Fixture;if(!$f -or $f.MarkerSha256 -ne $s.fixture.MarkerSha256 -or $f.Identity -ne $s.fixture.Identity -or $f.FirstRunComplete){throw 'Controlled pristine profile drift detected.'}
    $p=Get-Policies;if(!(Same $p.Related $s.related)){throw 'Related Edge policy drift detected.'};if((Get-StartupFolders).EdgeEntryCount -ne 0){throw 'Edge Startup-folder drift detected.'};if(!(Same (Get-Protected) $s.protected)){throw 'Protected service configuration drift detected.'}
}
...
try{
    ...
    switch($Action){
        ...
        'Rollback' {$s=Read-State;Assert-DriftFree $s;Assert-EdgeClosed;$now=Get-Reg $policyPath $valueName;if(!$now.ValueExists -and !$s.original.ValueExists){Write-Log 'rollback' 'idempotent' @{mutationCount=0};return [pscustomobject]@{RolledBack=$true;MutationCount=0}};if($s.original.ValueExists){if($now.ValueExists -and !(Same $now ([pscustomobject]@{KeyExists=$true;ValueExists=$true;Kind='DWord';Data=1}))){throw 'Policy drift detected; rollback overwrite refused.'};$kind=[Enum]::Parse([Microsoft.Win32.RegistryValueKind],[string]$s.original.Kind,$true);$k=Get-Item -LiteralPath $policyPath;$k.SetValue($valueName,$s.original.Data,$kind)}else{if($now.ValueExists -and !(Test-Applied)){throw 'Policy drift detected; rollback overwrite refused.'};if($now.ValueExists){Remove-ItemProperty -LiteralPath $policyPath -Name $valueName};if(!$s.original.KeyExists -and (Test-Path -LiteralPath $policyPath) -and @((Get-Item -LiteralPath $policyPath).GetValueNames()).Count -eq 0 -and @((Get-ChildItem -LiteralPath $policyPath -ErrorAction SilentlyContinue)).Count -eq 0){Remove-Item -LiteralPath $policyPath}};$after=Get-Reg $policyPath $valueName;if(!(Same $after $s.original)){throw 'Exact rollback verification failed.'};Write-Log 'rollback' 'pass' @{restoredExactOriginal=$true};[pscustomobject]@{RolledBack=$true;restoredExactOriginal=$true}}
    }
