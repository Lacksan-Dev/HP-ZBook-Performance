...
try{
 ...
 switch($Action){
  'Check'{$c=Get-Candidates;Write-Log 'candidate-inventory' 'pass' @{count=$c.Count;names=@($c.Name)};[pscustomobject]@{Support=$support;Candidates=$c;Profile='LogiBoltDemandLaunch'}}
  'Capture'{$s=Save-State $support (Get-Candidates);Write-Log 'capture' 'pass' @{path=$s.entry.Path;name=$s.entry.Name};$s}
  'DryRun'{$c=Get-Candidates;Assert-Eligible $support $c;$r=[pscustomobject]@{Profile='LogiBoltDemandLaunch';WouldChange=$true;MutationCount=1;Path=$c[0].Path;Name=$c[0].Name;PreserveApplication=$true;PreserveDrivers=$true;PreservePairing=$true;RebootPersistenceCheckRequired=$true;Rollback='Restore exact captured value name, kind, and unexpanded data.'};Write-Log 'dry-run' 'pass' $r;$r}
