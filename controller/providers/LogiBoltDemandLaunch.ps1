...
$experiment='EXP-051'
$provider='logi-bolt-demand-launch'
...
function Get-Hash([string]$Text){$s=[Security.Cryptography.SHA256]::Create();try{($s.ComputeHash([Text.Encoding]::UTF8.GetBytes($Text))|%{$_.ToString('x2')})-join''}finally{$s.Dispose()}}
...
function Get-SupportState{
 ...
 [pscustomobject]@{Supported=($o.Caption -match 'Windows 11' -and $c.Manufacturer -match '(?i)^HP$|Hewlett-Packard');OS=$o.Caption;Build=$o.BuildNumber;Manufacturer=$c.Manufacturer;Model=$c.Model;Managed=$m.Managed;ManagementSignals=$m.Signals;EvidenceStatus='needs-evidence'}
}
function Get-ProtectedSnapshot{
 $s=foreach($n in 'WinDefend','mpssvc','wuauserv','UsoSvc','BITS','Tailscale','TermService'){ $x=Get-Service $n -ErrorAction SilentlyContinue;if($x){[ordered]@{Name=$x.Name;Status=$x.Status.ToString();StartType=$x.StartType.ToString()}}}
...
function Resolve-Command([string]$Command){
 ...
 if($exe -notmatch '(?i)\\Logi(?:tech)?\\.*\\?(LogiBolt|logi-bolt)\.exe$' -and $exe -notmatch '(?i)\\LogiBolt\\.*\\?(LogiBolt|logi-bolt)\.exe$'){return $null}
 if($exe -match '(?i)updat|uninstall|repair|firmware'){return $null}
 if([string]::IsNullOrWhiteSpace($args) -or $args -notmatch '(?i)background|minimi[sz]ed|startup|tray|silent'){return $null}
...
function Get-Candidates{
 ...
 foreach($n in $k.GetValueNames()){
  if($n -notmatch '(?i)^logi(?:tech)?[ ._-]*bolt$|^logibolt$'){continue};$d=[string]$k.GetValue($n,$null,[Microsoft.Win32.RegistryValueOptions]::DoNotExpandEnvironmentNames);$r=Resolve-Command $d;if(!$r){continue};$id=Get-FileIdentity $r.Executable;if(!$id -or !$id.ValidLogitechPublisher){continue}
  $a=Get-Acl -LiteralPath $runPath;$out+=[pscustomobject]@{Path=$runPath;Name=$n;Kind=$k.GetValueKind($n).ToString();Data=$d;ExpandedCommand=$r.ExpandedCommand;Arguments=$r.Arguments;Executable=$id;KeyOwner=$a.Owner;KeySddl=$a.Sddl}
 };@($out)
}
function Assert-Eligible($Support,[object[]]$Candidates){if(!$Support.Supported){throw 'HP Windows 11 is required.'};if($Support.Managed){throw 'Enterprise-management ownership detected.'};if($Candidates.Count -ne 1){throw "Exactly one eligible Logi Bolt Run registration is required; found $($Candidates.Count)."}}
...
function Assert-Executable($State){$e=Get-FileIdentity ([string]$State.entry.Executable.Path);if(!$e -or !$e.ValidLogitechPublisher -or $e.Sha256 -ne [string]$State.entry.Executable.Sha256 -or $e.Thumbprint -ne [string]$State.entry.Executable.Thumbprint){throw 'Logi Bolt executable identity drift detected.'}}
